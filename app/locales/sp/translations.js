export default {

  /*
   * COMMON: Translations used globally in the app or shared between components
   */
   "common":{  
      "assessment-pl":{  
         "other":"Evaluaciones",
         "one":"Evaluación"
      },
      "categoryOptions":{  
         "professional-dev":"Desarrollo profesional",
         "higher-ed":"Educación más alta",
         "k12":"K-12"
      },
      "collection-pl":{  
         "other":"Colecciones",
         "one":"Colección"
      },
      "delete-instructions":{  
         "links-inaccessible":"Todos los enlaces compartidos serán inaccesibles",
         "content-inaccessible":"Todo el contenido será inaccesible a las aulas vinculadas a él"
      },
      "emotions":{  
         "emotion-3":"Meh ...",
         "emotion-2":"No entiendo",
         "emotion-5":"puedo explicarlo",
         "emotion-4":"entiendo",
         "emotion-1":"necesito ayuda"
      },
      "errors":{  
         "class-title-presence":"Por favor dé un nombre a su salón de clases.",
         "add-question-description":"Ingrese la pregunta.",
         "lesson-not-created":"¡Vaya! No se puede crear lección ahora mismo. Vuelve a intentarlo en breve.",
         "resource-same-host-url":"Los recursos no pueden ser URL de Gooru.",
         "add-username":"Por favor, ingrese un nombre de usuario.",
         "collection-not-created":"¡Vaya! No se puede crear colección ahora mismo. Vuelve a intentarlo en breve.",
         "resource-missing-type":"Seleccione un tipo de recurso.",
         "sign-up-name-only-letters":"Por favor, escriba sólo letras.",
         "lesson-title-required":"Introduzca el título de la lección.",
         "user-email-presence":"Por favor introduzca una dirección de correo electrónico válida.",
         "class-not-created":"¡Vaya! No se puede crear el aula ahora mismo. Vuelve a intentarlo en breve.",
         "lesson-not-copied":"¡Vaya! No se puede copiar la lección ahora mismo. Vuelve a intentarlo en breve.",
         "join-class-code":"Por favor ingrese el código de la clase.",
         "correct-answer-presence":"Por favor, indique la respuesta correcta.",
         "assessment-not-copied":"¡Vaya! No se puede copiar la evaluación ahora mismo. Vuelve a intentarlo en breve.",
         "password-length":"La contraseña debe tener entre 5 y 14 caracteres.",
         "add-question-answer-text":"Introduzca el texto de la respuesta.",
         "resource-missing-url":"Por favor introduzca un URL válido.",
         "unit-not-created":"¡Vaya! No se puede crear la unidad ahora mismo. Vuelve a intentarlo en breve.",
         "collection-not-added-to":"¡Vaya! No se puede agregar la colección a la lección ahora mismo. Vuelve a intentarlo en breve.",
         "resource-not-created":"¡Vaya! No se puede crear recursos ahora mismo. Vuelve a intentarlo en breve.",
         "file-upload-missing":"Selecciona un archivo con cualquiera de las siguientes extensiones: {{extensions}}",
         "sign-in-google-account-exists":"Utiliza el inicio de sesión de Google. No podemos restablecer su contraseña.",
         "forgot-password-gmail":"Utiliza el inicio de sesión de Google. No podemos restablecer su contraseña.",
         "add-course-title":"Ingrese el título del curso.",
         "course-not-updated":"¡Vaya! No se puede actualizar el curso ahora mismo. Vuelve a intentarlo en breve.",
         "password-special-characters":"No utilice caracteres especiales.",
         "answer-has-no-image":"Cargue una imagen de respuesta.",
         "sign-up-name-length":"El apellido debe tener al menos 2 letras.",
         "sign-up-valid-email":"Por favor, introduce una dirección de correo electrónico válida.",
         "course-not-created":"¡Vaya! No se puede crear curso ahora mismo. Vuelve a intentarlo en breve.",
         "special-characters":"No puede utilizar caracteres o espacios especiales.",
         "assessment-not-updated":"¡Vaya! No se puede actualizar la evaluación ahora mismo. Vuelve a intentarlo en breve.",
         "resource-description-length":"La descripción no puede tener más de 500 caracteres.",
         "password-not-match":"Las contraseñas no coinciden.",
         "password-required":"Porfavor ingrese una contraseña.",
         "resource-missing-title":"Introduzca un título de recurso.",
         "resource-not-updated":"¡Vaya! No se puede actualizar el recurso ahora mismo. Vuelve a intentarlo en breve.",
         "can-not-join-class":"¡Vaya! No se puede unir al aula. Vuelve a intentarlo en breve.",
         "highlight-text-wrong-format":"Formato incorrecto de la pregunta.",
         "resource-title-length":"El título no puede tener más de 50 caracteres.",
         "add-question-title":"Introduzca el título de la pregunta.",
         "question-not-added-to":"¡Vaya! No se puede agregar la pregunta a {{collectionType}} ahora mismo. Vuelve a intentarlo en breve.",
         "reset-password-error":"¡UH oh! Algo no esta bien. No se puede restablecer la contraseña. Vuelve a intentarlo en breve.",
         "highlight-text-not-selected":"Por favor, indique la respuesta correcta.",
         "assessment-not-created":"¡Vaya! No se puede crear evaluación en este momento. Vuelve a intentarlo en breve.",
         "file-max-size":"Sólo se admiten archivos de tamaño inferior a 5 MB",
         "hotspot-text-max-choices":"Ha alcanzado el límite de opciones de respuesta.",
         "profile-not-updated":"¡Vaya! No se puede actualizar el perfil ahora mismo. Vuelve a intentarlo en breve.",
         "collection-not-updated":"¡Vaya! No se puede actualizar la colección ahora mismo. Vuelve a intentarlo en breve.",
         "question-not-copied":"¡Vaya! No se puede copiar la pregunta ahora mismo. Vuelve a intentarlo en breve.",
         "resource-not-added-to-collection":"¡Vaya! No se puede agregar el recurso a la recopilación ahora mismo. Vuelve a intentarlo en breve.",
         "sign-up-error":"¡Vaya! No se puede registrar ahora mismo. Vuelve a intentarlo en breve.",
         "sign-up-last-name":"Por favor ingrese su apellido.",
         "lesson-not-loaded":"¡Vaya! No se puede cargar la lección ahora mismo. Vuelve a intentarlo en breve.",
         "resource-not-copied":"¡Vaya! No se puede copiar recurso ahora mismo. Vuelve a intentarlo en breve.",
         "unit-not-copied":"¡Vaya! No se puede copiar la unidad ahora mismo. Vuelve a intentarlo en breve.",
         "collection-not-copied":"¡Vaya! No se puede copiar la colección ahora mismo. Vuelve a intentarlo en breve.",
         "password-confirm":"Por favor, confirme su contraseña.",
         "course-not-copied":"¡Vaya! No se puede copiar el curso ahora mismo. Vuelve a intentarlo en breve.",
         "assessment-not-added-to":"¡Vaya! No se puede agregar la evaluación a la lección ahora mismo. Vuelve a intentarlo en breve.",
         "assessment-title-presence":"Ingrese el título de evaluación.",
         "collection-title-presence":"Introduzca el título de la colección.",
         "select-correct-answer":"Seleccione la respuesta correcta.",
         "search-collections-length":"Introduzca al menos 3 caracteres.",
         "unit-title-required":"Introduzca el título de la unidad.",
         "question-not-created":"¡Vaya! No se puede crear la pregunta ahora mismo. Vuelve a intentarlo en breve.",
         "reset-google-account-exists":"Su inicio de sesión de correo electrónico se creó con una cuenta de Google y no podemos restablecer una contraseña de Google. Si olvidaste tu contraseña de Google, tendrás que restablecerla a través de tus aplicaciones de Google.",
         "resource-invalid-url":"URL invalida.",
         "username-length":"El nombre de usuario debe tener entre 4 y 16 caracteres.",
         "question-not-updated":"¡Vaya! No se puede actualizar la pregunta ahora mismo. Vuelve a intentarlo en breve.",
         "sign-up-first-name":"Por favor, introduzca su nombre de pila.",
         "sign-in-credentials-not-valid":"¡UH oh! Algo no esta bien. Revisa tu nombre de usuario y contraseña y vuelve a intentarlo.",
         "unit-not-loaded":"¡Vaya! No se puede cargar la unidad ahora mismo. Vuelve a intentarlo en breve.",
         "class-min-score":"La puntuación mínima debe ser un número entre 1 y 100"
      },
      "lessonObj":{  
         "other":"Lecciones",
         "one":"Lección"
      },
      "numberStudents":{  
         "other":"{{count}} Estudiantes",
         "one":"{{count}} Estudiante"
      },
      "question-pl":{  
         "other":"Preguntas",
         "one":"Pregunta"
      },
      "question-type":{  
         "HT_TO":"Arrastrar y soltar orden",
         "MA":"Respuesta múltiple",
         "HS_IMG":"Selección múltiple - Imagen",
         "MC":"Opción multiple",
         "OE":"Respuesta libre",
         "HS_TXT":"Selección múltiple - Texto",
         "HT_HL":"Subrayar el texto",
         "T/F":"Verdadero o falso",
         "T_F":"Verdadero o falso",
         "SA":"Respuesta única",
         "FIB":"Llena el espacio en blanco",
         "HT_RO":"Arrastrar y soltar orden"
      },
      "resource-format":{  
         "image":"Imagen",
         "question":"Pregunta",
         "interactive":"Interactivo",
         "text":"Texto",
         "video":"Vídeo",
         "webpage":"Página web",
         "audio":"Audio"
      },
      "resource-pl":{  
         "other":"Recursos",
         "one":"Recurso"
      },
      "unitObj":{  
         "other":"Unidades",
         "one":"Unidad"
      },
      "warnings":{  
         "on-air-connection-lost":"El panel de Go Live ha perdido la conexión y está intentando de nuevo automáticamente. Es tentador, pero por favor no actualice su pantalla!",
         "character-limit":"Has alcanzado el límite de caracteres."
      },
      "cancel":"Cancelar",
      "add-units":"Añadir unidades",
      "depth-of-knowledge":"Profundidad del Conocimiento",
      "upload":"Subir",
      "about":"Acerca de",
      "search-error-message":"Los términos de búsqueda deben tener al menos 3 letras.",
      "update-thumbnail":"Actualizar miniatura",
      "explanation":"Explicación",
      "learning-target":"Micro-estándar",
      "new-collection":"Nueva colección",
      "score":"Puntuación",
      "add-to":"Añadir",
      "approved":"Aprobado",
      "password":"Contraseña",
      "launch-on-air":"Ir a vivir",
      "new-assessment":"Nueva Evaluación",
      "show-more-results":"Mostrar más resultados",
      "state":"Estado o territorio",
      "add-type-question":"¿Qué tipo de pregunta le gustaría agregar?",
      "add-new-lesson":"Crear nueva lección",
      "add-standards-to-collection":"Agregar normas a la recopilación",
      "audience":"Audiencia",
      "edit":"Editar",
      "profile":"Perfil",
      "from-my-resources":"De Mis Recursos",
      "remix-lesson-success":"Has remezclado una lección {{lessonTitle}}.",
      "completed":"Terminado",
      "first-name":"Nombre de pila",
      "remaining":"{{number}} Izquierda",
      "enter-url":"Introducir URL",
      "standards":"Estándares",
      "answer-correct":"¡Estás en lo correcto!",
      "from-my-collections":"De mis colecciones",
      "sortAverage":"Ordenar por Promedio",
      "resource-url":"URL del recurso",
      "remix-collection-success":"Has remezclado una colección {{collectionTitle}}. ¿Quieres editar esa colección?",
      "remix-question":"Pregunta de Remix",
      "domain":"Dominio",
      "answer-key-was-hidden":"Nota: Su profesor ha ocultado la tecla de respuesta.",
      "collectionInitial":"do",
      "launch-anonymous":"Lanzamiento Anónimo",
      "role":"Papel",
      "submit-all":"Enviar todo",
      "show-correct-answer":"Mostrar respuesta correcta",
      "add-collection-item":"Crear recurso o pregunta",
      "request-to":"Solicitud de revisión de una insignia",
      "mastery":"Maestría",
      "new-question":"Nueva pregunta",
      "search":"Buscar",
      "lesson-title":"Título de la Lección",
      "builder":"Editor",
      "collection-title":"Título de la colección",
      "confirm-copy":"Confirmar y copiar",
      "visibility-tooltip":"No visible para los demás",
      "close":"Cerca",
      "content-publishing":"Visibilidad del contenido",
      "last-name":"Apellido",
      "remix-assessment-success":"Has remezclado una evaluación {{assessmentTitle}}. ¿Quieres editar esa evaluación?",
      "remix-question-lead":"Estás a punto de remezclar una pregunta.",
      "add-from-url":"Agregar desde URL",
      "reaction":"Reacción",
      "assessment-external":"Evaluación externa",
      "select-a-framework":"Primero seleccione un Marco de Estándares en la sección de Información sobre el curso.",
      "no-collections-to-display":"No se mostrarán <span>colecciones</span> .",
      "no-questions-to-display":"No hay <span>preguntas</span> para mostrar.",
      "biography":"Biografía",
      "not-provided":"No proporcionado",
      "published-tooltip":"Contenido Badged",
      "back-to-data":"Volver a Datos",
      "answer":"Tu respuesta",
      "select-resource-type":"Seleccione el tipo de recurso",
      "information":"Información",
      "show-results":"Mostrar resultados",
      "a-resource":"un recurso",
      "no-more-attempts":"No más intentos",
      "send-request":"Enviar petición",
      "standard":"Estándar",
      "country":"País",
      "correct":"Correcto",
      "add-course":"Crear nuevo curso",
      "remix-lesson-lead":"Estás a punto de remezclar una lección.",
      "add-new-unit":"Crear nueva unidad",
      "questions":"Preguntas",
      "no-assessments-to-display":"No hay <span>evaluaciones</span> que mostrar.",
      "advanced-editing":"Edición avanzada",
      "remix-assessment-lead":"Estás a punto de remixar una evaluación.",
      "save-finish":"Guardar y terminar",
      "assessment":"Evaluación",
      "edit-assessment":"Editar evaluación",
      "not-following":"No estás siguiendo a nadie.",
      "select-question-type":"Seleccione el tipo de pregunta",
      "school":"Colegio",
      "back-to-course-map":"Volver al Mapa del Curso",
      "edit-collection":"Editar colección",
      "finish":"Terminar",
      "featured-courses":"Cursos destacados",
      "add":"Añadir",
      "courses":"Cursos",
      "search-placeholder":"Buscar...",
      "unitInitial":"Tu",
      "if_questions":"Si tienes alguna pregunta,",
      "add-to-lesson-success":"Has añadido {{collectionTitle}} a {{lessonTitle}}. ¿Desea editar ese {{collectionType}}?",
      "collection":"Colección",
      "last-updated":"Última actualización",
      "current-attempt":"Intento actual",
      "district":"Distrito",
      "remix-assessment":"Evaluación Remix",
      "report-in-progress":"Informe en curso",
      "unexpectedError":"Se ha producido un error inesperado y se ha informado. ¡Lo sentimos por las molestias!",
      "support":"Apoyo",
      "unfollow":"No seguir",
      "next":"Siguiente",
      "nothing-to-display":"Nada que mostrar.",
      "preview":"Avance",
      "correct-answer":"Respuesta correcta",
      "latest-attempt":"Último intento",
      "remix-collection":"Colección Remix",
      "avatarFor":"Avatar para",
      "submit":"Enviar",
      "sortAlphabetical":"Ordenar alfabéticamente",
      "classes":"Aulas",
      "pending":"Pendiente",
      "back":"Espalda",
      "description":"Descripción",
      "unit-title":"Título de la Unidad",
      "untitled-lesson":"Lección sin título",
      "content":"Contenido",
      "remove":"retirar",
      "course-map":"Mapa del curso",
      "no-results":"No se han encontrado resultados",
      "remix-resource-success":"Has remezclado un recurso {{resourceTitle}}. ¿Quieres editar ese recurso?",
      "create-class":"+ Crear un aula",
      "request-report":"Solicitar informe",
      "class":"Aula",
      "drag-drop-suggestions":"O Sugerencias de arrastrar y soltar ...",
      "an-assessment":"Una evaluación",
      "yes":"Sí",
      "teach":"Enseñar",
      "false":"Falso",
      "libraries":"Bibliotecas",
      "valid-extensions":"Las extensiones de archivo válidas son: {{extensions}}",
      "remix-course":"Curso Remix",
      "no-content":"No hay contenido disponible",
      "remix-question-success":"Has remezclado una pregunta {{questionTitle}}. ¿Quieres editar esa pregunta?",
      "not-specified":"No especificado",
      "home":"Casa",
      "add-standard":"Añadir Estándar",
      "file-name":"Nombre del archivo",
      "edit-question":"Editar pregunta",
      "created-by":"Creado por",
      "remix-lesson":"Lección Remix",
      "assign":"Asignar",
      "play":"Jugar",
      "download-report":"Descargar informe",
      "select":"Seleccionar",
      "copy-to":"Copiar a",
      "no-followers":"Aún no tienes seguidores.",
      "from-my-questions":"De mis preguntas",
      "best-practices":"Mejores prácticas",
      "no-resources-to-display":"No hay <span>recursos</span> para mostrar.",
      "a-course":"un curso",
      "suggested-resources":"Recursos sugeridos",
      "from-my-assessments":"De mis evaluaciones",
      "type":"Tipo",
      "attempt":"Número de intento",
      "signUp":"Regístrate",
      "add-type-resource":"¿Qué tipo de recurso es este?",
      "number":"No.",
      "logout":"Cerrar sesión",
      "essential-questions":"Preguntas esenciales",
      "remix-resource":"Recurso Remix",
      "search-standards":"Normas de búsqueda",
      "add-question-image":"Añadir imagen de pregunta",
      "collections":"Colecciones",
      "no-results-message":"Corrige tu ortografía. ¡Todos cometemos errores! <br/> Ir más amplio y quitar algunos filtros. <br/> O intente buscar una palabra similar en su lugar.",
      "remixed-by":"Remixado por",
      "add-collaborator":"Añadir Colaborador",
      "new-question-text":"Introduzca el texto de la pregunta aquí",
      "untitled-course":"Curso 1",
      "sentence":"Frase",
      "contentUnavailable":"Contenido no disponible",
      "contributed-by":"Contribuido por",
      "more-details":"Más detalles",
      "timeSpent":"Tiempo usado",
      "hints":"Sugerencias",
      "lesson":"Lección",
      "not-added":"No añadido",
      "loading":"Cargando ...",
      "save-submit":"Guardar y enviar todo",
      "a-collection":"Una colección",
      "lessonInitial":"L",
      "anonymous_mode":"Modo anónimo",
      "following":"Siguiendo",
      "school-info":"Información de la escuela",
      "add-url":"Agregar URL",
      "link-out-message":"* Si su recurso no está representado en la vista previa anterior, puede que necesite un 'link-out' y no se pueda ver dentro del reproductor Gooru.",
      "remix-resource-lead":"Estás a punto de remixar un recurso.",
      "word":"Palabra",
      "remix":"Remix",
      "other":"Otro",
      "study":"Estudiar",
      "notStarted":"No empezado",
      "save":"Salvar",
      "remix-unit-success":"Has remixado una unidad {{unitTitle}}.",
      "add-new-resource":"Crear nuevo recurso",
      "login":"Iniciar sesión",
      "subject-and-framework":"Asignatura y marco",
      "add-assessment":"Crear una nueva evaluación",
      "add-question":"Crear pregunta",
      "add-collection":"Crear nueva colección",
      "averageScore":"Puntuación media",
      "untitled-unit":"Untitled Unit",
      "question-title":"Título de la pregunta",
      "forgotPassword":"Se te olvidó tu contraseña",
      "add-domains-to-unit":"Agregar dominios a la unidad",
      "course":"Curso",
      "copy":"Dupdo",
      "add-to-collection-success":"Has añadido {{contentTitle}} a {{collectionTitle}}. ¿Desea editar ese {{collectionType}}?",
      "question":"Pregunta",
      "course-title":"Título del curso",
      "resource-title":"Título del recurso",
      "menu":"Menú",
      "off":"APAGADO",
      "assign-to-course":"Asignar al curso",
      "save-next":"Guardar y Siguiente",
      "search-400-error-message":"Introduzca un término de búsqueda válido",
      "upload-file":"Subir archivo",
      "classScores":"Puntuaciones de clase",
      "edit-course":"Editar curso",
      "add-standards":"Añadir normas",
      "remix-warning":"¡Aviso! Hay un montón de contenido impresionante en este curso y hacer una copia llevará tiempo. Confirme que desea iniciar el proceso y en 15 minutos encontrará su copia de este curso en su <b>perfil.</b>",
      "a-question":"una pregunta",
      "remix-unit-lead":"Estás a punto de remixar una unidad.",
      "publish-to":"Hacer esto visible para otros en mi biblioteca de perfiles",
      "assessment-disabled":"Usted no puede intentar esta evaluación",
      "student-id":"ID del estudiante (no se muestra en el perfil)",
      "start-tour":"Hacer un tour",
      "create":"Crear",
      "from":"de",
      "please_contact":"Por favor contactar",
      "published-by":"Publicado por",
      "settings":"Ajustes",
      "add-lessons":"Añadir lecciones",
      "resource":"Recurso",
      "swap":"Reordenar",
      "assign-to-class":"Asignar al aula",
      "domains":"Dominios",
      "remix-collection-lead":"Estás a punto de remixar una colección.",
      "myContent":"Mi contenido",
      "edit-resource":"Editar Recurso",
      "license":"Licencia",
      "unit":"Unidad",
      "followers":"Seguidores",
      "true":"Cierto",
      "publisher":"Editor",
      "remix-course-success":"Has remezclado un curso {{courseTitle}}. ¿Quieres editar ese curso?",
      "big-ideas":"Grandes ideas",
      "no":"No",
      "hide-results":"Ocultar resultados",
      "student":"Estudiante",
      "upload-thumbnail":"Subir miniatura",
      "remix-course-lead":"Estás a punto de remezclar un curso.",
      "about-you":"Acerca de ti",
      "delete":"Borrar",
      "add-content-prompt":"No has creado <span>{{type}}</span> todavía. Sigue siendo audaz.",
      "email_support":"Support@gooru.org",
      "teacher":"Profesor",
      "of":"de",
      "beta":"Beta",
      "on":"EN",
      "assessments":"Evaluaciones",
      "move":"Movimiento",
      "assessmentInitial":"UN",
      "assessment-title":"Título de la evaluación",
      "link-out":"Vincular a",
      "verified":"Verificado",
      "resources":"Recursos",
      "toggle-dropdown":"Desactivar desplegable",
      "follow":"Seguir",
      "use-case":"Caso de uso",
      "learning-target-mobile":"Micro-estándar en estándar",
      "remix-unit":"Unidad Remix",
      "learning-objectives":"Objetivos de aprendizaje",
      "performance":"Mostrar el rendimiento",
      "edit-narration":"Editar Narración",
      "narration":"Narración",
      "answer-incorrect":"Eres incorrecto ...",
      "no-courses-to-display":"No hay <span>cursos</span> para mostrar.",
      "category":"Categoría",
      "add-new-question":"Crear una nueva pregunta",
      "new-resource":"Nuevo recurso"
   },
   "index":{  
      "browseContent":{  
         "footer":{  
            "description_1":"¿Ya tienes una cuenta?",
            "description_2":"aquí.",
            "login":"Iniciar sesión"
         },
         "button":"Navegar contenido",
         "description_3":"o",
         "description_1":"Busco",
         "description_2":"Materiales de aprendizaje",
         "title":"¡Hola! ¿Qué estás buscando?",
         "grades_missing_message":"Seleccione Grado y Asunto.",
         "subjects_missing_message":"Por favor, seleccione Asunto."
      },
      "gettingStarted":{  
         "toolkit":{  
            "description":"¡Bienvenido a Gooru! Echa un vistazo a estos recursos para aprender lo que puedes hacer con Gooru y empezar rápidamente.",
            "title":"Kit de herramientas de introducción"
         },
         "classroom":{  
            "description":"Aprenda con ejemplos a través de historias de maestros que dicen que Gooru ha hecho una diferencia en su salón de clases.",
            "title":"Historias del aula"
         },
         "events":{  
            "description":"Ofrecemos seminarios y seminarios gratuitos para ayudarle a comenzar con Gooru.",
            "title":"Echa un vistazo a nuestros eventos!"
         },
         "title":"Introducción a Gooru"
      },
      "empowerStudents":{  
         "find":"Encontrar",
         "share":"Compartir",
         "monitor":"Monitor",
         "title":"Capacitar a los estudiantes para que aprendan a su manera",
         "remix":"Remix"
      },
      "freeAndOpen":{  
         "button":"Más información sobre nuestro enfoque",
         "description":"Creemos que la educación es un derecho humano. Gooru siempre estará libre de costo y anuncios para educadores y estudiantes de todo el mundo.",
         "title":"Libre y abierto. <br/> Siempre."
      },
      "remixDescription":"Recupere colecciones y personalice contenido para satisfacer las necesidades de sus estudiantes.",
      "joinUs":"Únase a nosotros para <br/> Honrar el derecho humano a <br/> Educación",
      "findDescription":"Navegue por miles de colecciones de K-12 hechas por profesores, o busque más de 16 millones de recursos",
      "monitorDescription":"Mida el compromiso y el progreso de sus estudiantes para que intervengan en tiempo real.",
      "shareDescription":"Compartir colecciones con estudiantes a través de las aulas de Gooru. El acceso no es necesario para acceder."
   },
   "class":{  
      "info":{  
         "edit-info":"editar informacion",
         "share-class":"Compartir Clase",
         "teachers":"Maestros",
         "subject":"Tema",
         "grade":"Grado",
         "class-code":"Código de aula",
         "students":"Estudiantes",
         "description":"Descripción",
         "add-students":"Añadir Estudiantes",
         "delete":"Eliminar sala de clase",
         "class-info":"Información del Aula",
         "invite-co-teachers":"Invitar a los Co-maestros"
      },
      "edit":{  
         "basic-info":"Información básica",
         "class-greetings":"Anuncios de aula",
         "class-minscore":"Calificación Puntuación mínima para los trofeos (1-100%)",
         "edit-class":"Editar configuración del aula",
         "assigned-course":"Curso asignado",
         "class-name":"Nombre del aula",
         "class-greetings-placeholder":"Salude a sus estudiantes, motivarlos, hacer un anuncio, etc.",
         "course-map":"Mapa del curso"
      },
      "overview":{  
         "locate":"Ubícame",
         "editContent":"Editar contenido",
         "title":"Mapa del curso"
      },
      "analytics":{  
         "performance":{  
            "actions":{  
               "assessment":"Ver evaluación",
               "download":"Descargar",
               "edit":"Editar contenido",
               "exitFullScreen":"Salir de pantalla completa",
               "share":"Compartir",
               "collection":"Ver Colección",
               "fullScreen":"Ver en pantalla completa",
               "both":"Ver ambos"
            },
            "teacher":{  
               "metricsTable":{  
                  "average":"Promedio",
                  "class-average":"Promedio de clase"
               }
            },
            "title":"Ver rendimiento",
            "better-experience-message":"Para una mejor experiencia de Gooru, vea el análisis de aula completo en tableta o escritorio."
         },
         "mastery":{  
            "title":"Ver dominio"
         }
      },
      "quick-start":{  
         "new-collection":"Nueva colección",
         "featured-course":"Ver cursos destacados",
         "remix-desc":"Copia y personaliza un curso destacado para tus estudiantes.",
         "remix-a-sample":"Remix una muestra",
         "new-assessment":"Nueva Evaluación",
         "existing-course-desc":"La forma más rápida de comenzar un aula",
         "new-course-desc":"Comience creando una colección o evaluación",
         "add-existing-course":"Agregar un curso de su biblioteca",
         "choose-course":"Elija Curso",
         "title":"Asignar contenido a este aula.",
         "new-course":"Inicio rápido de un nuevo curso",
         "remix-from-course":"Remix de un curso destacado"
      }
   },
   "classes":{  
      "classesJoined":"Aulas a las que me he unido",
      "noClassesJoined":"No te has unido a ninguna clase",
      "classesTaught":"Aulas que enseño",
      "noClassesTaught":"No tienes clases creadas"
   },
   "content":{  
      "assessments":{  
         "edit":{  
            "best-practices":"<p> Una evaluación es un conjunto de preguntas puntuadas que usted y sus alumnos pueden usar para monitorear el entendimiento y el desempeño. </p><p> Use una variedad de tipos de preguntas (incluyendo varias basadas en SBAC) en su evaluación para que los estudiantes puedan demostrar comprensión de diferentes maneras. Recomendamos etiquetar cada pregunta a estándares, micro-estándares y Profundidad de conocimiento de Webb. </p>"
         }
      },
      "classes":{  
         "create":{  
            "condition-prompt":"¿Cómo se unirán los estudiantes a su aula?",
            "get-started":"Empezar",
            "condition-prompt-invite":"Sólo invitados",
            "condition-prompt-code":"Cualquier persona con código de Clase",
            "class-name-input":"Nombre su aula",
            "title":"Crear un aula",
            "content":"Donde los estudiantes se involucran con el contenido."
         },
         "join":{  
            "already-member":"Ya eres miembro de este salón de clases.",
            "join-not-allowed":"No puedes unirte a este aula",
            "terms-and-conditions":"Al hacer clic en Unirse al aula, estoy de acuerdo en compartir mis datos de evaluación y recopilación de datos generados a partir del estudio de este salón Gooru con el (los) maestro (s) de este salón.",
            "invalid-code":"Código de salón no válido.",
            "class-not-found":"Aula no encontrada. Asegúrese de haber introducido el código correcto en el aula",
            "join-class":"Únete al aula",
            "title":"Únete a un aula",
            "not-now":"Ahora no",
            "class-code-input":"Introduzca un código de salón de clases",
            "content":"Donde comienza el viaje."
         }
      },
      "collections":{  
         "edit":{  
            "assign-to-course":"Asignar al curso",
            "best-practices":"<p> Los estudiantes interactúan con su contenido en el nivel de colección. Al crear una colección de aprendizaje, asegúrese de incluir los objetivos de aprendizaje y considere el uso de una variedad de tipos de recursos para exponer a los estudiantes a los conceptos de múltiples maneras. </p><p> Utilice la secuenciación de los recursos para desarrollar conceptos. La progresión a través de una colección debe fluir de una manera lógica y llevar a la audiencia prevista de un nivel general a un nivel más complejo de comprensión si es apropiado, o permitir adecuadamente para la exploración del estudiante. </p><p> Incluya chequeos de comprensión a lo largo del camino a través de nuestras preguntas Gooru u otros interactivos. Recomendamos recursos suficientes y / o suficientes recursos para lograr los objetivos de la recolección y asegurar que cada recurso tenga un papel y propósito. </p>"
         }
      },
      "courses":{  
         "edit":{  
            "information":{  
               "course-title":"Título del curso",
               "description":"Descripción"
            },
            "assign-to-class":"Asignar al aula",
            "best-practices":"<p> Un curso es una carpeta que le permite organizar su contenido de aprendizaje en unidades y lecciones. Al crear un curso, considere las preguntas esenciales que está abordando, los objetivos de aprendizaje y la organización de su contenido. </p><p> Puede juntar las lecciones para crear una experiencia diversa para su población estudiantil (por ejemplo, puede ordenar sus unidades cronológicamente, por tema o por estándar). </p>"
         }
      },
      "questions":{  
         "edit":{  
            "information":{  
               "question-title":"Título de la pregunta",
               "question-type":"tipo de pregunta"
            },
            "builder":{  
               "answer-instructions":{  
                  "HT_HL_WD":"Al escribir la pregunta, utilice los corchetes para las palabras resaltadas. Un soporte sólo puede contener una palabra a la vez. Por ejemplo, el [grande] lobo malo sopló [hacia abajo] la casa. Límite de caracteres: 5000.",
                  "MA":"Puede agregar hasta diez respuestas, una imagen, una explicación y hasta cinco sugerencias.",
                  "HS_IMG":"Puede agregar hasta diez imágenes de respuesta y seleccionar una o más respuestas correctas.",
                  "MC":"Puede agregar hasta diez opciones de respuesta e indicar una respuesta correcta. Límite de caracteres: 200.",
                  "OE":"Escriba la respuesta correcta. Límite de caracteres: 5000.",
                  "HS_TXT":"Puede agregar hasta diez opciones de respuesta y seleccionar una o más respuestas correctas.",
                  "T/F":"Selecciona la respuesta correcta.",
                  "FIB":"Añadir hasta 5 sugerencias para la respuesta y una explicación.",
                  "HT_HL_ST":"Al escribir la pregunta, utilice los corchetes para indicar las oraciones resaltadas. Un soporte sólo puede contener una oración a la vez, utilizando un período dentro del corchete. Por ejemplo, El primer pequeño cerdo construyó su casa de paja. El gran lobo arruinó la casa. El segundo cerdo construyó su casa de madera. Límite de caracteres: 5000.",
                  "HT_RO":"Puede agregar hasta diez opciones de respuesta en el orden correcto. La orden será codificada para los estudiantes."
               },
               "question-instructions":{  
                  "MA":"Escriba su pregunta.",
                  "HS_IMG":"Escriba su pregunta.",
                  "MC":"Escriba su pregunta.",
                  "OE":"Escriba su pregunta.",
                  "HS_TXT":"Escriba su pregunta.",
                  "HT_HL":"Escriba el mensaje de pregunta.",
                  "T/F":"Escriba su pregunta.",
                  "FIB":"Al escribir la pregunta, utilice los corchetes para sus respuestas de relleno en blanco. Por ejemplo: 'El gran mal [lobo] sopló la casa'. También puedes añadir una imagen.",
                  "HT_RO":"Escriba su pregunta."
               },
               "add-hint":"Añadir sugerencias",
               "answer":"Responder",
               "add-explanation":"Añadir explicación",
               "add-answer-choice":"+ Responder respuesta Choice"
            },
            "add-to":"Añadir",
            "best-practices":"<p> Una pregunta es un recurso que requiere una respuesta del estudiante, y ofrecemos una variedad de tipos de preguntas para apoyar el tipo de preguntas que sus estudiantes verán en SBAC, PARCC y otras evaluaciones. </p><p> Considere la posibilidad de alternar los tipos de preguntas que usted utiliza para ofrecer a los estudiantes la exposición a estos tipos de preguntas y para proporcionar múltiples formatos para demostrar el conocimiento. </p><p> Etiquetar sus preguntas a las normas, micro-estándares, y Profundidad del conocimiento de Webb. Usted puede ver cómo sus estudiantes están interactuando con preguntas a través del tablero del profesor. </p>"
         }
      },
      "modals":{  
         "delete-class":{  
            "student-data-deleted":"Se borrarán todos los datos de los estudiantes",
            "legend":"Estás a punto de eliminar tu aula",
            "student-access":"Los estudiantes no podrán acceder al aula"
         },
         "delete-content":{  
            "delete-error":"¡Vaya! No se puede eliminar {{type}} ahora mismo. Vuelve a intentarlo en breve.",
            "legend":"Estás a punto de eliminar",
            "content-legend":"<span>{{Type}}</span> {{index}} - {{title}} de {{parentName}}",
            "delete-warning":"Todo el contenido de este {{type}} se eliminará",
            "content-legend-header":"{{title}} de {{parentName}}",
            "confirmation":"Estás seguro de que quieres continuar? Por favor, escriba &quot;eliminar&quot; a continuación y haga clic en &quot;eliminar&quot;."
         },
         "delete-resource":{  
            "delete-error":"¡Vaya! No se puede eliminar {{type}} ahora mismo. Vuelve a intentarlo en breve.",
            "legend":"Confirma que deseas eliminar permanentemente <b>{{title}}</b>",
            "second-check":"Se eliminarán copias de este recurso, en sus colecciones y en cualquier colección de otros usuarios de la comunidad",
            "delete-warning":"Todo el contenido de este {{type}} se eliminará",
            "confirmation":"Estás seguro de que quieres continuar? Haga clic en 'Eliminar permanentemente'.",
            "first-check":"Se trata de una eliminación permanente y no se puede deshacer"
         },
         "remove-content":{  
            "legend":"Estás a punto de eliminar <b>{{title}}</b> de <b>{{parentName}}</b>",
            "remove-error":"¡Vaya! No se puede eliminar {{type}} ahora mismo. Vuelve a intentarlo en breve.",
            "confirmation":"Estás seguro de que quieres continuar? Por favor escriba &quot;remove&quot; y haga clic en &quot;remove&quot;."
         },
         "remove-student":{  
            "data-lost":"Si vuelven a unirse a la clase, todos los datos pasados ​​se perderán",
            "legend":"Estás a punto de eliminar {{studentName}} de este salón y eliminar todos sus datos.",
            "classroom-access":"No tendrán acceso al aula ni al contenido",
            "data-inaccessible":"Todos sus datos serán borrados y no accesibles por usted o por ellos",
            "remove-error":"¡Vaya! No se puede eliminar este estudiante ahora mismo. Vuelve a intentarlo en breve.",
            "confirmation":"Estás seguro de que quieres continuar? Por favor, escriba &quot;eliminar&quot; a continuación y haga clic en &quot;eliminar&quot;.",
            "title":"Eliminar al estudiante y eliminar sus datos"
         },
         "quick-remove-content":{  
            "legend":"Confirma que deseas eliminar <b>{{title}}</b> de <b>{{parentName}}</b> ."
         },
         "quick-delete-content":{  
            "legend":"Confirma que deseas eliminar permanentemente <b>{{title}}</b> de <b>{{parentName}}</b> .",
            "delete":"Borrar permanentemente"
         }
      },
      "resources":{  
         "edit":{  
            "information":{  
               "im-publisher":"Soy el editor",
               "select-a-license":"Seleccione una licencia"
            },
            "best-practices":"<p> Los recursos son contenido multimedia en una variedad de formatos como videos, interactivos, sitios web, imágenes, Google Docs y más. Sea creativo y utilice sus propios recursos o obtenga 'recursos' y busque en nuestra amplia oferta en Gooru. </p><p> Utilice una variedad de tipos de recursos para involucrar a sus estudiantes e incluir narración para que pueda ayudar a guiar a sus estudiantes a través del recurso. </p><p> Recomendamos etiquetar cada pregunta a estándares, micro-estándares y habilidades del siglo XXI. Usted puede ver cómo sus estudiantes están interactuando con los recursos a través del tablero del profesor. </p>",
            "placeholder-message":"Añade un recurso para <span>verlo aquí.</span>",
            "not-implemented":"La vista previa del formato de recursos <span>no se ha implementado todavía.</span>"
         }
      }
   },
   "user":{  
      "joined-classes":{  
         "other":"Actualmente estás inscrito en aulas de {{count}}",
         "one":"Actualmente estás inscrito en 1 aula"
      },
      "teaching-classes":{  
         "other":"Y clases de enseñanza {{count}}",
         "one":"Y la enseñanza de 1 aula"
      },
      "archived-classes":"Aulas archivadas",
      "create-class":"Crear un aula",
      "join-class":"Únete al aula",
      "active-classes":"Aulas Activas"
   },
   "gru-add-to":{  
      "add-to-assessment-lead":"Elija una evaluación que desea agregar {{contentTitle}} a",
      "add-collection-to-lesson-lead":"Seleccione una colección para agregar a esta lección.",
      "add-assessment-to-lesson-lead":"Seleccione una evaluación para agregar a esta lección.",
      "add-to-collection-lead":"Elige una colección que quieras agregar {{contentTitle}} a",
      "collections-info":"Las colecciones que aparecen aquí <b>no</b> pertenecen a otra lección o curso",
      "add-to-collection":"Añadir a la colección",
      "add-to-assessment":"Añadir a la valoración o recaudación",
      "assessments-info":"Las evaluaciones enumeradas aquí <b>no</b> pertenecen a otra lección o curso",
      "add-assessment-to-lesson":"Añadir de mis evaluaciones",
      "add-collection-to-lesson":"Añadir de mis colecciones"
   },
   "gru-assessment-confirmation":{  
      "attempts-left":{  
         "other":"Tienes intentos de {{count}}",
         "one":"Tienes 1 intento a la izquierda"
      },
      "setting-forward-backward":"Puede navegar hacia adelante y hacia atrás para responder preguntas",
      "cancel":"Cancelar",
      "setting-forward":"Solo puede navegar hacia adelante",
      "unlimited-attempts-left":"Tienes intentos ilimitados",
      "start":"¡Comienzo!",
      "description":"En esta evaluación, {{model.title}}",
      "unlimited-attempts":"Tienes intentos ilimitados",
      "title":"Estás a punto de comenzar una evaluación ..."
   },
   "gru-submit-confirmation":{  
      "cancel":"Cancelar",
      "confirm":"Enviar todo",
      "description":"Estás a punto de terminar este intento y presentar todas las respuestas. Cualquier pregunta omitida se contará como incorrecta.",
      "title":"¿Está seguro de que desea enviar todo?"
   },
   "gru-quick-course-search":{  
      "view-featured-courses":"Ver cursos destacados",
      "add-from-course":"Agregar desde el curso existente",
      "assign":"Asignar"
   },
   "gru-share-pop-over":{  
      "ios-tooltip":"Mantenga pulsado para copiar!",
      "multiarch-tooltip":"Presiona Ctrl + C para copiar!",
      "share-question":"Comparta su pregunta con el enlace",
      "safari-osx-tooltip":"¡Presione Cmd + C para copiar!",
      "share-collection":"Comparte tu colección con el enlace",
      "share-assessment":"Comparta su evaluación con el enlace",
      "copy":"Dupdo",
      "share-course":"Comparte tu curso con el enlace",
      "share-resource":"Comparte tu recurso con el enlace"
   },
   "gru-category-panel":{  
      "teacher":{  
         "cta":"Ver historias",
         "title":"Para maestros",
         "body":"Descubra el contenido alineado con las normas, personalice el contenido y realice el seguimiento del progreso de los estudiantes a través del análisis de datos."
      },
      "student":{  
         "cta":"Entrar",
         "text-placeholder":"Introduzca el código del salón de clases",
         "title":"Para estudiantes",
         "body":"Explore los intereses, construya y monitoree el progreso a través de materiales de aprendizaje."
      },
      "district":{  
         "cta":"Vea Nuestro Impacto",
         "title":"Para Distritos",
         "body":"Colabore con Gooru para dar rienda suelta al aprendizaje personalizado y compartir un plan de estudios aprobado por el distrito."
      },
      "partner":{  
         "cta":"Aprende más",
         "title":"Para socios",
         "body":"Colaborar con socios de misión para aumentar nuestro impacto colectivo en el ecosistema educativo."
      }
   },
   "class.gru-class-navigation":{  
      "analytics":"Datos",
      "overview":"Mapa del curso",
      "teams":"Equipos",
      "members":"Miembros",
      "active":"Activo:",
      "information":"Información del Aula",
      "greetings":"Anuncios"
   },
   "gru-user-registration":{  
      "signUpEmail":"Regístrese con su dirección de correo electrónico",
      "joinTitle":"¡Únase a la comunidad de Gooru!",
      "clickLogIn":"Haga clic aquí para ingresar.",
      "or":"O",
      "joinDescription":"Encuentra, remix y comparte los mejores recursos de aprendizaje K-12 gratuitos.",
      "googleButton":"Regístrese con Google",
      "noGoogleAccount":"¿No tienes una cuenta de Google?",
      "whyGoogle":"¿Por qué registrarse en Google?",
      "descriptionWhyGoogle":"Es rápido y fácil. Utiliza tu cuenta de Google existente para iniciar sesión sin contraseña.",
      "haveAccount":"¿Ya tienes una cuenta?"
   },
   "sign-up":{  
      "dateOfBirth":{  
         "months":{  
            "november":"noviembre",
            "june":"junio",
            "september":"septiembre",
            "may":"Mayo",
            "august":"agosto",
            "january":"enero",
            "february":"febrero",
            "july":"julio",
            "december":"diciembre",
            "october":"octubre",
            "april":"abril",
            "march":"marzo"
         },
         "month":"Mes",
         "error-message":"Por favor, introduce tu fecha de nacimiento.",
         "year":"Año",
         "title":"Cumpleaños",
         "day":"Día"
      },
      "step-child-subtitle":"No podemos completar su registro.",
      "error-email-taken":"Este correo electrónico se ha tomado. Prueba otro.",
      "google-button":"Regístrese con Google",
      "error-role-message":"Seleccione un rol.",
      "step-child-title":"¡No tan rapido!",
      "error-username-taken":"Aww, este nombre de usuario es tomado. Prueba otro.",
      "step-child-age-requirements":"Términos y condiciones",
      "step-1-title":"¡Hola!",
      "error-state-message":"Seleccione su estado o territorio.",
      "step-child-description-2":". Sigue aprendiendo y te veré en unos pocos años!",
      "step-child-description-1":"Gooru no pudo crear su cuenta debido a nuestra",
      "step-2-description":"No eres básico, pero esta información es.",
      "password":"Contraseña",
      "step-1-description":"Nos alegra que haya decidido unirse a nosotros.",
      "log-in-description":"Si ya tiene una cuenta.",
      "rePassword":"Confirmar contraseña",
      "error-country-message":"Por favor seleccione su país.",
      "district":"Organización del distrito o de la carta",
      "log-in":"Iniciar sesión",
      "state":"Estado o territorio",
      "error-district-message":"Por favor, seleccione su distrito / charter de la lista o proporcionarlo en 'Otro'.",
      "email":"Email",
      "step-2-title":"Información básica",
      "username":"Nombre de usuario"
   },
   "gru-user-sign-up-cancel":{  
      "registration_incomplete":"Su registro no está completo.",
      "exit?":"¿Seguro que quieres salir?",
      "leave":"Dejar el registro",
      "continue":"Continuar con Registro",
      "title":"¿Dejar el registro?"
   },
   "login":{  
      "description-session-ends":"Por Favor regístrese.",
      "or":"O",
      "signUpHere":"¡Registrate aquí!",
      "description":"El aprendizaje está a la vuelta de la esquina.",
      "title":"¡Dar una buena acogida!",
      "gooruAccountTitle":"Inicie sesión en su cuenta de Gooru",
      "title-session-ends":"Su sesión ha caducado.",
      "haveAccount":"¿Tiene usted una cuenta?",
      "forgotPassword":"¿Olvidaste tu contraseña?",
      "password":"Contraseña",
      "googleButton":"Inicia sesión con Google",
      "log-in":"Iniciar sesión",
      "usernameOrEmail":"Nombre de usuario o correo electrónico"
   },
   "forgot-password":{  
      "mail":"Support@gooru.org",
      "footer-description-1":"Recibirá un correo electrónico con un enlace para restablecer su contraseña.",
      "error-email-not-exists":"Lo sentimos, no reconocemos este correo electrónico.",
      "footer-description-2":"Si tiene alguna pregunta, póngase en contacto con",
      "secondStepDescription-1":"Te hemos enviado un correo electrónico con un enlace para restablecer tu contraseña.",
      "description":"Nos pasa a todos.",
      "secondStepDescription-2":"Si tiene alguna pregunta, póngase en contacto con",
      "footer-google-description-1":"Prueba a iniciar sesión de nuevo pulsando <a href='/sign-in'>'Iniciar sesión con Google'.</a>",
      "usernameOrEmail":"Por favor introduzca su correo electrónico",
      "secondStepTitle":"Consultar su correo electrónico"
   },
   "reset-password":{  
      "new-password-confirm":"Confirmar la contraseña",
      "new-password":"Introduzca su nueva contraseña",
      "title":"Restablecer la contraseña"
   },
   "footer":{  
      "contactUs":"Contáctenos",
      "districts":"Distritos",
      "privacy":"Intimidad",
      "community":"Comunidad",
      "aboutGooru":"Acerca de Gooru",
      "coaches":"Entrenadores",
      "Copyright":"Derechos de autor",
      "careers":"Carreras",
      "partners":"Fogonadura",
      "terms":"Condiciones",
      "legal":"Legal",
      "company":"Empresa",
      "supportCenter":"Centro de Apoyo",
      "footerDescription":"Gooru se ha comprometido a mantener su plataforma de código abierto y comunidad CC0 contenido creado.",
      "connect":"Conectar",
      "events":"Eventos"
   },
   "grade-dropdown":{  
      "tenth":"10",
      "eighth":"8",
      "eleventh":"11",
      "k":"K",
      "second":"2",
      "sixth":"6",
      "third":"3",
      "seventh":"7",
      "twelfth":"12",
      "middle-school":"Escuela intermedia",
      "elementary":"Elemental",
      "ninth":"9",
      "fifth":"5",
      "placeholder":"Grado (s)",
      "high-school":"Escuela secundaria",
      "higher-ed":"Ed más alto",
      "fourth":"4",
      "prompt":"Seleccione un grado",
      "pre-k":"Pre-k",
      "first":"1"
   },
   "standard-dropdown":{  
      "placeholder":"Navegar por Estándar"
   },
   "subject-dropdown":{  
      "placeholder":"asignaturas)",
      "prompt":"Seleccione un tema"
   },
   "search-filter":{  
      "question-types":{  
         "MA":"Respuesta múltiple",
         "HS_IMG":"Selección múltiple - Imagen",
         "MC":"Opción multiple",
         "HS_TXT":"Selección múltiple - Texto",
         "HT_HL":"Texto en caliente",
         "T/F":"Verdadero Falso",
         "FIB":"Llena el espacio en blanco",
         "HT_RO":"Arrastrar y soltar orden"
      },
      "author":{  
         "placeholder":"Autor"
      },
      "courses":"Cursos",
      "assessments":"Evaluaciones",
      "collections":"Colecciones",
      "questions":"Preguntas",
      "resources":"Recursos"
   },
   "resource":{  
      "image":"Imagen",
      "question":"Pregunta",
      "interactive":"Interactivo",
      "video":"Vídeo",
      "webpage":"Página web",
      "text":"Texto",
      "audio":"Audio",
      "oer":"REA"
   },
   "search-result":{  
      "question":"Pregunta",
      "in-this-collection":"En esta colección",
      "resource":"Recurso",
      "and":"y",
      "questions":"Preguntas",
      "resources":"Recursos",
      "search-results-for":"Buscar resultados para"
   },
   "gru-image-picker":{  
      "chooseFile":"Escoge un archivo ...",
      "submit":"Usar la imagen",
      "instruction":"Cargue una imagen de un archivo en su computadora.",
      "restriction":"La imagen debe ser un archivo JPG, GIF o PNG inferior a 5 MB."
   },
   "gru-fib":{  
      "instructions":"Escriba su respuesta en el espacio en blanco proporcionado y haga clic en '{{action}}'."
   },
   "gru-hs-image":{  
      "instructions":"Seleccione la imagen correcta y haga clic en '{{action}}'."
   },
   "gru-hs-text":{  
      "instructions":"Seleccione la respuesta correcta y haga clic en '{{action}}'."
   },
   "gru-hot-text":{  
      "instructions":"Seleccione la respuesta correcta y haga clic en '{{action}}'."
   },
   "gru-login-prompt":{  
      "new-user":"¿Nuevo aquí?",
      "instructions":"Debe iniciar sesión para completar esta acción.",
      "sign-in":"Registrarse",
      "title":"¡Bienvenido a Gooru!",
      "existing-user":"¿Ya tienes una cuenta?",
      "not-now":"Ahora no"
   },
   "gru-multiple-answer":{  
      "instructions":"Seleccione la respuesta correcta y haga clic en '{{action}}'."
   },
   "gru-multiple-choice":{  
      "instructions":"Seleccione la respuesta correcta y haga clic en '{{action}}'."
   },
   "gru-open-ended":{  
      "instructions":"Escribe tu respuesta en el campo siguiente y haz clic en el botón '{{action}}' para guardar tu respuesta cuando hayas terminado.",
      "characterLimit":"Límite de caracteres"
   },
   "gru-question-viewer":{  
      "answer":"Responder",
      "question":"Pregunta"
   },
   "gru-true-false":{  
      "instructions":"Seleccione la respuesta correcta y haga clic en '{{action}}'.",
      "true":"Cierto",
      "false":"Falso"
   },
   "gru-reorder":{  
      "instructions":"Reorganice las respuestas en el orden correcto y haga clic en '{{action}}'."
   },
   "player":{  
      "gru-navigation":{  
         "view-report":"Vista del informe"
      },
      "gru-navigator":{  
         "see-usage-report":"Ver el informe de uso"
      },
      "gru-viewer":{  
         "not-iframe-url":{  
            "view-resource":"Ver Recurso",
            "header_2":"Haga clic en el botón de abajo para abrir el recurso en una nueva pestaña.",
            "footer_4":"Requisitos que lo llevan a otra página para ver el contenido.",
            "footer_3":"Crear y compartir su contenido. Los recursos tienen una variedad de",
            "header_1":"Este recurso no se puede ver en Gooru.",
            "footer_2":"Los recursos añadidos en Gooru provienen de miles de editores diferentes que",
            "footer_1":"¿Por qué estoy viendo esta página en blanco?"
         }
      }
   },
   "profile":{  
      "gru-navigation":{  
         "library":"Biblioteca",
         "followers":"Seguidores",
         "about":"Acerca de",
         "about-you":"Acerca de ti"
      },
      "edit":{  
         "select-district":"Seleccione un distrito ..."
      }
   },
   "gru-data-picker":{  
      "score":"Puntuación",
      "completion":"Terminación",
      "study-time":"Tiempo de estudio",
      "reaction":"Reacción",
      "timeSpent":"Hora",
      "attempts":"Intento"
   },
   "gru-performance-summary":{  
      "resume":"Currículum",
      "completion":"Terminación",
      "study":"Estudia ahora",
      "reaction":"Reacción",
      "scores":"Puntuaciones",
      "view-report":"Vista del informe",
      "not-started":"No ha comenzado todavía",
      "redo":"Rehacer",
      "title":"Título",
      "not-applicable":"N / A",
      "time-spent":"Tiempo Total",
      "attempts":"Intentos"
   },
   "gru-performance":{  
      "no-content":"No hay contenido disponible"
   },
   "gru-metrics-sub-header":{  
      "score":"Puntuación",
      "completion":"Terminación",
      "study-time":"Tiempo de estudio",
      "student":"Estudiante"
   },
   "gru-resource-new":{  
      "resource-already-exist":"Este recurso ya existe en Gooru!"
   },
   "gru-assessment-report":{  
      "gru-summary":{  
         "total-time-spent":"Tiempo total gastado"
      },
      "hidden-report":"Su profesor ha seleccionado ocultar su informe de resumen para esta evaluación."
   },
   "cards":{  
      "gru-class-card":{  
         "student":{  
            "other":"{{count}} Estudiantes",
            "one":"{{count}} Estudiante"
         },
         "archived":{  
            "download-report":"Descargue sus datos para esta clase.",
            "report-in-progress":"La generación de informes puede tardar hasta 20 min. Por favor, vuelva.",
            "request-report":"Esta clase está archivada y no se puede modificar. Se puede acceder a los datos de clase existentes mediante el informe.",
            "no-report-available":"Esta clase no tiene contenido de curso asignado."
         }
      },
      "gru-course-card":{  
         "units":{  
            "other":"Unidades {{count}}",
            "one":"Unidad {{count}}"
         },
         "resource":{  
            "other":"{{Count}} Recursos",
            "one":"{{Count}} Recurso"
         },
         "question":{  
            "other":"{{Count}} Preguntas",
            "one":"{{Count}} Pregunta"
         },
         "in":"en",
         "and":"y"
      },
      "gru-resource-card":{  
         "add":"Añadir"
      },
      "gru-resource-result-card":{  
         "skipped":"Omitido"
      },
      "gru-profile-card":{  
         "followers":"Seguidores",
         "following":"Siguiendo"
      },
      "gru-user-network-card":{  
         "follow":"Seguir"
      }
   },
   "reports.gru-table-view":{  
      "score":"Puntuación",
      "study-time":"Tiempo de estudio",
      "reaction":"Reacción",
      "student":"Estudiante",
      "scores":"Puntuaciones",
      "first-tier-header-prefix":"Q",
      "reactions":"Reacciones",
      "totals":"Total",
      "time-spent":"Tiempo usado"
   },
   "gru-emotion-picker":{  
      "react-to-resource":"Reaccionar a este recurso"
   },
   "home":{  
      "no-classes-found":{  
         "create-class":{  
            "button-text":"Crear un aula",
            "description":"Crear un aula, asignar contenido e invitar a los estudiantes.",
            "title":"Enseñar con una clase de Gooru"
         },
         "join-class":{  
            "button-text":"Introduzca el código del salón de clases",
            "description":"Únase al salón de clases de su maestro para comenzar a aprender.",
            "title":"Aprender con un aula de Gooru"
         },
         "featured-courses":{  
            "button-text":"Cursos destacados",
            "description":"Examine los cursos de matemáticas, ciencias, estudios sociales y ELA.",
            "title":"Cursos destacados"
         },
         "teacher-toolkit":{  
            "button-text":"Kit de herramientas del profesor",
            "description":"Este juego de herramientas tiene recursos para ayudarle a empezar.",
            "title":"Kit de herramientas del profesor"
         }
      }
   },
   "taxonomy":{  
      "gru-taxonomy-selector":{  
         "primary-subject-and-course":"Marco y Curso de Normas",
         "add-secondary":"Agregar secundario",
         "choose-subject":"Elija el asunto"
      }
   },
   "validations":{  
      "unsavedChanges":"Tus cambios aún no se han guardado. ¿Quieres dejar esta página?"
   },
   "featured":{  
      "featured-title":"Cursos destacados",
      "featured-description":"Los cursos ofrecidos por Gooru son examinados y revisados, educados por el educador, creados en las aulas y estudiados por los estudiantes. Ellos fueron desarrollados e implementados en escuelas, distritos y charters innovadores, y están diseñados para apoyar el aprendizaje combinado, las aulas volteadas, el aprendizaje basado en proyectos y muchos otros modelos de instrucción. Descubre, remezcla y personaliza cursos para personalizar el aprendizaje y aumentar el compromiso estudiantil. Haga clic aquí para <a href='http://about.gooru.org/courses' target='_blank'>obtener más información</a> sobre el desarrollo de estos cursos."
   },
   "taxonomy.modals":{  
      "gru-domain-picker":{  
         "selectedText":{  
            "other":"{{Count}} dominios seleccionados",
            "one":"{{Count}} dominio seleccionado"
         },
         "shortcutText":"Curso está en",
         "browseSelectorText":"¿Qué dominios cubrirá esta unidad?"
      },
      "gru-standard-picker":{  
         "selectedText":{  
            "other":"{{Count}} estándares seleccionados",
            "one":"{{Count}} estándar seleccionado"
         },
         "shortcutText":"La unidad está etiquetada como",
         "browseSelectorText":"¿Qué estándares serán cubiertos?"
      }
   },
   "account-settings":{  
      "birthday":"Cumpleaños",
      "email-address":"Dirección de correo electrónico",
      "gender":"Género",
      "account-info":"Informacion de cuenta",
      "title":"Configuraciones de la cuenta",
      "private-info":"Información Privada"
   },
   "gru-rich-text-editor":{  
      "expressions-panel":{  
         "tabs":{  
            "layout":"Diseño",
            "trigonometry":"Trigonometría",
            "set-theory":"Teoría de conjuntos",
            "greek-letters":"Letras griegas",
            "calculus":"Cálculo",
            "symbols":"Símbolos",
            "relation":"Relación"
         },
         "update-expression":"Actualizar",
         "insert-expression":"Insertar",
         "create-expression":"Crear expresión"
      },
      "expression":"Expresión",
      "subscript":"Subíndice",
      "underline":"Subrayar",
      "superscript":"Sobrescrito",
      "bold":"Negrita",
      "italic":"Itálico"
   },
   "gru-settings-edit":{  
      "feedback":"Los estudiantes verán si son correctos / incorrectos",
      "disable-heading":"Activar evaluación en el mapa del curso",
      "navigation-scoring":"Navegación y Puntuación",
      "feedback-immediate":"Por pregunta y al final",
      "answerkey-attempts":"Tecla de respuesta y intentos",
      "disable-legend":"Los estudiantes pueden jugar la evaluación desde su mapa de cursos",
      "feedback-never":"Nunca",
      "feedback-summary":"Al final",
      "answer-key":"Los estudiantes pueden ver la tecla de respuesta al final",
      "backwards":"Los estudiantes pueden navegar hacia atrás y cambiar las respuestas",
      "attempts":"Intentos",
      "attempts-unlimited":"Ilimitado"
   },
   "gru-tour":{  
      "overview":{  
         "stepOne":{  
            "description":"El mapa de cursos proporciona a sus estudiantes acceso a todas las evaluaciones y colecciones que les asignan.",
            "title":"Mapa del curso"
         },
         "stepTwo":{  
            "description":"Cada aula que crees tiene un código de clase único. Le dará este código a los estudiantes cuando esté listo para que se unan a su salón de clases y accedan a su contenido.",
            "title":"Código de clase"
         },
         "stepThree":{  
            "description":"Esto le permite ver los datos de evaluación de la clase y los estudiantes individuales cuando los estudiantes completan las evaluaciones que son parte de un curso.",
            "title":"Monitorear Datos de Estudiantes y Clases"
         },
         "stepFour":{  
            "description":"Aquí puede editar su nombre de salón de clases, publicar anuncios para sus estudiantes, ver los nombres de los estudiantes matriculados en su clase y eliminar su salón de clases.",
            "title":"Información del Aula"
         },
         "stepFive":{  
            "description":"Cuando usted está en un salón de clases, haga clic aquí para editar cualquier contenido del curso asignado a sus estudiantes.",
            "title":"Edición del contenido del curso"
         },
         "stepSix":{  
            "description":"Utilice el panel de control en tiempo real para supervisar el progreso de la clase en una evaluación en tiempo real. <br><br> Haga clic en el icono &quot;Go Live&quot; que se encuentra a la izquierda de cada evaluación para lanzar una evaluación en tiempo real para los estudiantes. <br><br>",
            "title":"¡Supervise el progreso en tiempo real!"
         }
      },
      "quick-start":{  
         "stepOne":{  
            "description":"Esta es una vista de un aula recién creada. Para regresar a un aula en cualquier momento, haga clic en &quot;Aulas&quot; y use el menú desplegable para seleccionar el aula que desea ingresar.",
            "title":"Navegando por sus aulas"
         },
         "stepTwo":{  
            "description":"Sugerimos crear una evaluación como una forma de empezar con Gooru y evaluar los niveles actuales de comprensión del estudiante en su clase.",
            "title":"¿Empezando? Crear una evaluación!"
         }
      },
      "home":{  
         "stepOne":{  
            "description":"¡Bienvenidos a la página de inicio de Gooru! Aquí puede encontrar una lista de las aulas que usted crea en Gooru. Las aulas le permiten compartir contenido directamente con los estudiantes. Siempre puedes volver a tu página principal haciendo clic en el icono de Gooru.",
            "title":"Su Página de Inicio"
         },
         "stepTwo":{  
            "description":"Todas las aulas que enseña o se une a este año escolar aparecerán aquí bajo Aulas Activas.",
            "title":"Clases en las que enseña o se une"
         },
         "stepThree":{  
            "description":"Todas las aulas que creó o se unió en los últimos años escolares se pueden encontrar en aulas archivadas. El contenido que se usa en estas aulas se puede encontrar en su perfil y se puede volver a asignar a nuevas aulas para este año escolar.",
            "title":"Clases anteriores"
         },
         "stepFour":{  
            "description":"Haga clic aquí para crear un nuevo aula. Una vez que tenga contenido para compartir con los estudiantes, se lo asignará a través de un aula.",
            "title":"Crear un aula"
         },
         "stepFive":{  
            "description":"Este es tu perfil. Haz clic en tu perfil en cualquier momento para acceder al contenido que creas o remixas en Gooru.",
            "title":"Tu perfil"
         }
      }
   }
};
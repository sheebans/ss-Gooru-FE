...
export default RemixBaseModal.extend({
  ...
  showSuccessNotification: function(question) {
    var component = this;
    var successMsg = component.get('i18n').t('common.remix-question-success', {questionTitle: question.get('title')});
    var questionEditUrl = component.get('router').generate('content.questions.edit', question.get('id'));
    var edit = component.get('i18n').t('common.edit');
    component.get('notifications').success(`${successMsg} <a class="btn btn-success" href="${questionEditUrl}">${edit}</a>`);
  },

  showFailureNotification: function() {
    const message = this.get('i18n').t('common.errors.question-not-copied').string;

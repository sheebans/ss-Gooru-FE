...
export default Ember.Component.extend({
  ...
  performance: null,
  /**
   * @property {Array} assessmentItems information
   */
  ...
  assessmentInfo: Ember.observer('performance', function() {
    let locations = this.get('locations');
    let performance = this.get('performance');
    ...
    let assessmentData = locations.map(location => ({
      location,
      performance: performanceMap[location.courseId || location.collectionId]
    }));
    this.set('assessmentItems', assessmentData);

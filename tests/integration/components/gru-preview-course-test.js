...
test('Preview Course Layout - teacher', function(assert) {
  ...
  assert.ok($collectionCard.find('.course-information .course-info .remixed-by .title').length, 'Owner title is missing');
  assert.ok($collectionCard.find('.course-information .course-info .remixed-by img').length, 'Missing image');
  assert.ok($collectionCard.find('.course-information .course-info .remixed-by .owner').length, 'Missing owner name');
  assert.ok($collectionCard.find('.description').length, 'Missing course description');
  assert.ok($collectionCard.find('.units').length, 'Missing units panel');
  assert.equal($collectionCard.find('.units .unit').length,2, 'Should have 2 units');
...
test('Preview Course Layout - student', function(assert) {
  ...
  assert.ok($collectionCard.length, 'Missing Component');
  assert.ok($collectionCard.find('.actions .cancel').length, 'Missing cancel button');
  assert.ok($collectionCard.find('.actions .start-btn').length, 'Missing start studying button');
});

...
test('Preview Collection Layout - teacher', function(assert) {
  ...
  assert.notOk($collectionCard.find('.header-action .bookmark-btn').length, 'Bookmark should not appear');
  assert.ok($collectionCard.find('.collection-information .image img').length, 'Missing Image');
  assert.ok($collectionCard.find('.collection-information .collection-info .title-section h3.title').length, 'Missing Title');
  assert.ok($collectionCard.find('.collection-information .collection-info .standards .gru-taxonomy-tag-list').length, 'Missing Standards');
  assert.ok($collectionCard.find('.collection-information .collection-info .remixed-by .title').length, 'Owner title is missing');
  assert.ok($collectionCard.find('.collection-information .collection-info .remixed-by img').length, 'Missing image');
  assert.ok($collectionCard.find('.collection-information .collection-info .remixed-by .owner').length, 'Missing owner name');
  assert.ok($collectionCard.find('.description').length, 'Missing collection description');
  assert.ok($collectionCard.find('.resources').length, 'Missing resources panel');

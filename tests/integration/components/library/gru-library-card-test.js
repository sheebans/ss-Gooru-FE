...
moduleForComponent('library/gru-library-card', 'Integration | Component | library/gru library card', {
  integration: true
});
...
test('Library card', function(assert) {
  ...
  assert.ok($component.find('.information .title .resource-count'), 'Missing resource count');
  ...
  assert.ok($component.find('.panel-footer .network'), 'Missing network section');
  assert.ok($component.find('.panel-footer .actions .share-btn'), 'Missing share button');
  assert.ok($component.find('.panel-footer .actions .bookmark-btn'), 'Missing bookmark button');
});

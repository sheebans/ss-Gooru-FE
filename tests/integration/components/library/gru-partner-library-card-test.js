...
test('Partner Library card', function(assert) {
  ...
  assert.ok($body.find('.description').length, 'Missing partner library description');
  assert.equal(
  ...
  assert.equal(
    $info.find('.total-courses span').text().trim(),
    '5',
    'Missing courses count'
  );
  assert.equal(
    $info.find('.total-collections span').text().trim(),
    '15',
    'Missing collections count'
  );
  assert.equal(
    $info.find('.total-assessments span').text().trim(),
    '12',
  ...
  assert.equal(
    $info.find('.total-questions span').text().trim(),
    '45',

...
test('it renders', function (assert) {
  ...
  this.render(hbs`{{charts/gru-completion-chart color=color completed=completed total=total}}`);
  ...
  assert.equal($bar.find('.segment:nth-child(1)').attr("style"), "background-color: #DD0000; width: 40%;", "Segment");
...
test('it renders correctly when the total value is zero', function (assert) {
  ...
  this.render(hbs`{{charts/gru-completion-chart completed=completed total=total}}`);

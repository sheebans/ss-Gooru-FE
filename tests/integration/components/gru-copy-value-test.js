...
moduleForComponent('gru-copy-value', 'Integration | Component | gru copy value', {
  integration: true,
  beforeEach: function () {
...
test('Layout', function(assert) {
  ...
  T.exists(assert, $copyButton.find('.gru-icon.content_copy'), 'Missing correct icon');

  assert.equal($inputHidden.val(), "value-to-copy", "Wrong value to be copied");
  assert.equal($copyButton.attr('data-original-title'), "Click to copy class code", "Wrong tooltip for button");

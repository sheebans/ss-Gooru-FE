...
moduleForComponent('cards/gru-student-class-card', 'Integration | Component | cards/gru student class card', {
  integration: true,
  beforeEach: function () {
...
test('Class Card Layout', function(assert) {
  ...
  assert.equal(T.text($panelHeading.find('h5')), 'My class - 1', 'Wrong class title text');

  T.exists(assert, $panelHeading.find('.code'), 'Missing class card code');
  assert.equal(T.text($panelHeading.find('.code span')), 'VZFMEWH', 'Wrong class code text');

  T.exists(assert, $panelBody.find('.charts .charts.gru-bubble-chart'), 'Missing gru-bubble-chart component');
  assert.equal(T.text($panelBody.find('.charts .charts.gru-bubble-chart .bubble-circle span')), '90%', 'Wrong performance score of the chart');
  assert.equal(T.text($panelBody.find('.charts .col-md-6:eq(0) span:eq(1)')), this.get('i18n').t('student-landing.class.performance').string, 'Wrong legend of the performance chart');

  T.exists(assert, $panelBody.find('.charts .charts.gru-radial-chart'), 'Missing gru-radial-chart component');
  assert.equal(T.text($panelBody.find('.charts .charts.gru-radial-chart text')), '50%', 'Wrong completed score of the chart');

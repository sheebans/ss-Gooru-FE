...
test('Resource Card Published', function(assert) {
  ...
  this.set('resource', resource);
  this.render(hbs`{{cards/gru-resource-card resource=resource allowProfileNavigation=true publishVisible=true}}`);
  var $component = this.$(); //component dom element
  const $resourceCard = $component.find(".gru-resource-card");
  T.exists(assert, $resourceCard.find(".panel-heading .publish-icon"), "Missing publish icon");
});

...
test('Resource Card Layout', function(assert) {
  ...
  this.render(hbs`{{cards/gru-resource-card resource=resource allowProfileNavigation=true}}`);
...
test('Question Card Layout', function(assert) {
  ...
  this.render(hbs`{{cards/gru-resource-card resource=question allowProfileNavigation=true}}`);
  var $component = this.$(); //component dom element
  const $resourceCard = $component.find(".gru-resource-card");
  T.exists(assert, $resourceCard.find(".panel-heading h6.title"), "Missing Title");
...
test('Resource Card with publisher', function(assert) {
  ...
  T.notExists(assert, $resourceCard.find(".panel-body .publisher .owner a"), "The publisher name should not be a link");
...
test('Not allow profile navigation', function(assert) {
  ...
  this.set('resource', resource);
  this.render(hbs`{{cards/gru-resource-card resource=resource allowProfileNavigation=false}}`);
  var $component = this.$(); //component dom element
  const $resourceCard = $component.find(".gru-resource-card");
  T.notExists(assert,$resourceCard.find(".panel-body .publisher .publisher-name"), "Publisher Name should not be visible");
  T.exists(assert, $resourceCard.find(".panel-body .publisher img"), "Publisher should not have image");
  T.notExists(assert, $resourceCard.find(".panel-body .publisher .owner a.enable"), "The publisher name should not be a link");
  T.exists(assert,$resourceCard.find(".panel-body .publisher .owner a.disabled"), "Missing owner Name");
});

test('Do not show profile information', function(assert) {
  var resource = ResourceModel.create({
    title: "Resource Title",
    format:"video",
    description:"Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.",
  ...
  this.set('resource', resource);
  this.render(hbs`{{cards/gru-resource-card resource=resource showProfileInformation=false}}`);
  var $component = this.$(); //component dom element
  const $resourceCard = $component.find(".gru-resource-card");
  T.notExists(assert, $resourceCard.find(".panel-body .publisher img"), "Publisher image should not be visible");
  T.notExists(assert, $resourceCard.find(".panel-body .publisher .owner"), "The publisher name should not be visible");
});

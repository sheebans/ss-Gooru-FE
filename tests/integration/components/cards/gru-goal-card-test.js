...
test('Goal Card Layout', function(assert) {
  ...
  T.notExists(assert, $panelBody.find(".reflection"), "Reflection should not be visible");
  ...
  T.exists(assert, $panelFooter.find(".expand-collapse i.keyboard_arrow_down"), "Missing expand icon");
...
test('Goal Card Layout - expanded', function(assert) {
  ...
  T.exists(assert, $panelFooter.find(".expand-collapse i.keyboard_arrow_up"), "Missing collapse icon");
});
...
test('Goal Card - expand collapse', function(assert) {
  ...
  T.exists(assert, $goalCard, "Missing collapsed goal card section");
  T.notExists(assert, $goalCard.find(".reflection"), "Reflection should not be visible");
  T.exists(assert, $goalCard.find(".expand-collapse i.keyboard_arrow_down"), "Missing expand icon");

  $goalCard.find(".expand-collapse").click(); //click the arrow

  return wait().then(function(){
    T.exists(assert, $component.find(".gru-goal-card.expanded"), "Missing expanded goal card");
    T.exists(assert, $goalCard.find(".reflection"), "Missing reflection");
    T.exists(assert, $goalCard.find(".expand-collapse i.keyboard_arrow_up"), "Missing collapse icon");

    $goalCard.find("> .collapse").click(); //click at the blue bar
    return wait().then(function() {
      T.exists(assert, $component.find(".gru-goal-card.collapsed"), "Missing collapsed goal card section");
      T.notExists(assert, $goalCard.find(".reflection"), "Reflection should not be visible");
      T.exists(assert, $goalCard.find(".expand-collapse i.keyboard_arrow_down"), "Missing expand icon");
    });
  });

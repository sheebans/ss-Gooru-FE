...
test('Hot Spot Image Correct Answer', function(assert) {
  ...
  var question = Ember.Object.create({
    ...
    assetBasePath: "http://qacdn.gooru.org/qalive/f000/2441/3272/",
    answers:  Ember.A([
      Ember.Object.create({ id: "1", isCorrect: true, text:"4a12b833-9106-48ef-95b5-ac3ff73575ae.png" }),
  ...
  this.render(hbs`{{reports/assessment/questions/gru-hs-image question=question showCorrect=showCorrect}}`);
  const $component = this.$(); //component dom element
  const $hsImage = $component.find(".reports.assessment.questions.gru-hs-image");
...
test('Hot Spot Image Your Answer Incorrect', function(assert) {
  ...
  this.render(hbs`{{reports/assessment/questions/gru-hs-image question=question userAnswer=userAnswer}}`);
  const $component = this.$(); //component dom element
  const $hsImage = $component.find(".reports.assessment.questions.gru-hs-image");
  ...
  T.exists(assert, $hsImage.find('li:eq(2) span.no-selected'), 'The third answer should not be selected');
});
...
test('Hot Spot Image Your Answer Correct', function(assert) {
  ...
  T.exists(assert, $hsImage.find('li:eq(2) span.selected.correct'), 'The third answer should be correct and selected');
  T.notExists(assert, $hsImage.find('li span.incorrect'), 'Should not be incorrect answers at all');
});
...
test('Hot Spot Image anonymous', function(assert) {
  ...
  this.render(hbs`{{reports/assessment/questions/gru-hs-image question=question userAnswer=userAnswer anonymous=true}}`);
  const $component = this.$(); //component dom element
  const $hsImage = $component.find(".reports.assessment.questions.gru-hs-image");

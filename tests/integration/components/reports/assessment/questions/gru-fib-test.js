...
moduleForComponent('reports/assessment/questions/gru-fib', 'Integration | Component | reports/assessment/questions/gru-fib', {
  integration: true
});
...
test('Fill in the blank Correct Answer', function(assert) {
  ...
  this.render(hbs`{{reports/assessment/questions/gru-fib question=question showCorrect=showCorrect}}`);
...
test('Fill in the blank Your Answer Incorrect', function(assert) {
  ...
  this.render(hbs`{{reports/assessment/questions/gru-fib question=question userAnswer=userAnswer}}`);
  const $component = this.$(); //component dom element
  const $fib = $component.find(".reports.assessment.questions.gru-fib");
  T.exists(assert, $fib, 'Missing fill in the blank component');
...
test('Fill in the blank anonymous', function(assert) {
  ...
  this.render(hbs`{{reports/assessment/questions/gru-fib question=question userAnswer=userAnswer anonymous=true}}`);
  const $component = this.$(); //component dom element
  const $fib = $component.find(".reports.assessment.questions.gru-fib");
  T.exists(assert, $fib, 'Missing fill in the blank component');

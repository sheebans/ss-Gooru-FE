...
test('Hot Spot Text Correct Answer', function(assert) {
  ...
  this.render(hbs`{{reports/assessment/questions/gru-hs-text question=question showCorrect=showCorrect}}`);
...
test('Hot Spot Text Your Answer Incorrect', function(assert) {
  ...
  this.render(hbs`{{reports/assessment/questions/gru-hs-text question=question userAnswer=userAnswer}}`);
  const $component = this.$(); //component dom element
  const $hsText = $component.find(".reports.assessment.questions.gru-hs-text");

...
test('Questions Details Mobile Layout', function(assert) {
  ...
  this.render(hbs`{{reports/assessment/gru-resources-xs results=resourceResults}}`);
  const $component = this.$(); //component dom element
  const $resource = $component.find(".gru-resources-xs");

  T.exists(assert, $resource, 'Missing resources-xs component');
  T.exists(assert, $resource.find('.resource-number'), 'Missing resource number column');
  T.exists(assert, $resource.find('.resource-text'), 'Missing resource text column');
  T.exists(assert, $resource.find('.resource-container'), 'Missing resource container');
  T.exists(assert, $resource.find('.resource-container .image'), 'Missing resource icon');

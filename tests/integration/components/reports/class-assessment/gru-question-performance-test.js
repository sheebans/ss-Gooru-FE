...
test('Non anonymous, not started', function (assert) {
  ...
  T.exists(assert, $chart.find(".ratio"), "Missing gru-x-bar-chart");
  assert.equal(T.text($chart.find('.ratio')), '0/3', "Wrong completion text");
...
test('Non anonymous, layout', function (assert) {
  ...
  T.exists(assert, $chart.find(".gru-x-bar-chart"), "Missing gru-x-bar-chart");
  T.exists(assert, $chart.find(".ratio"), "Missing gru-x-bar-chart");
  assert.equal(T.text($chart.find('.ratio')), '3/3', "Wrong completion text for selected question");
...
test('Non anonymous, multiple choice', function (assert) {
  const selectedQuestion = Ember.Object.create({ //Multiple Choice
    "id": "56a120483b6e7b090501d3e7",
    questionType: 'MC',
    text: 'Sample Question MC',

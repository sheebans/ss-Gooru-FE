...
test('Layout', function(assert) {
  ...
  const $timePeriod = $performanceFilterPanelBody.find('.filter-type.time-period');
  ...
  const $coursePanel = $performanceFilterPanelBody.find('.content-panel.course');
  assert.ok($coursePanel.length,'Missing course panel');

  const $activityPanel = $performanceFilterPanelBody.find('.content-panel.activity');
  ...
  const $lessonPanel = $performanceFilterPanelBody.find('.content-panel.lesson');
...
test('Expand/Collapse Activity content panel', function(assert) {
  ...
  assert.equal($performanceFilterPanel.find('.filter-type.activity .filter-category').text().trim(), 'Study', 'Study filter selected');

  const $activityFilterTypeArrow = $performanceFilterPanel.find('.filter-type.activity .arrow');

  const $activityContentPanel = $performanceFilterPanel.find('.content-panel.activity');
  assert.ok($activityContentPanel.hasClass('hidden'), 'Activity content panel is hidden by default');

  $activityFilterTypeArrow.click();
  return wait().then(function () {
    assert.ok($activityContentPanel.hasClass('visible'), 'Activity content panel should be visible');
    assert.equal($performanceFilterPanel.find('.filter-type.activity .filter-category').text().trim(), 'Activity', 'Activity filter selected');
    assert.ok($activityContentPanel.find('.activity-option.study.active'), 'Study should be active');
    $activityFilterTypeArrow.click();
    return wait().then(function () {
      assert.equal($performanceFilterPanel.find('.filter-type.activity .filter-category').text().trim(), 'Study', 'Activity filter selected');
      assert.ok($activityContentPanel.hasClass('hidden'), 'Activity content panel is visible');
    });
  });
});

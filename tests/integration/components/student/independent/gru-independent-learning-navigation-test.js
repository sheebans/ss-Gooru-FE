...
moduleForComponent('student/independent/gru-independent-learning-navigation', 'Integration | Component | student/independent/gru independent-learning navigation', {
  integration: true,
  beforeEach: function () {
...
test('Independent learning Navigation', function(assert) {
  ...
  this.render(hbs`{{student.independent.gru-independent-learning-navigation selectedMenuItem='collections'}}`);
  ...
  T.exists(assert, $navigation.find('.nav .collections'), 'Missing collections link');
  T.exists(assert, $navigation.find('.nav .assessments'), 'Missing assessments link');

  //$menu item Selected
  T.exists(assert, $navigation.find('.collections.active'), 'Missing selected collections item');
...
test('Layout when a menu Item is selected', function(assert) {
  ...
  this.render(hbs`{{student.independent.gru-independent-learning-navigation onItemSelected='itemSelected'}}`);
  ...
  assert.equal($navigation.find('.nav .tab.active').length, 1, 'The menu should have only one item selected');
});

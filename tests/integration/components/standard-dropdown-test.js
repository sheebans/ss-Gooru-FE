...
test('Layout and interaction', function (assert) {
  ...
  this.render(hbs`{{standard-dropdown standards=items onStandardSelectedAction='myStandardSelectedAction'}}`);
  ...
  assert.equal($dropdownMenu.find("li a.item strong").length, 3, "Missing item name");
  assert.equal($dropdownMenu.find("li a.item p").length, 3, "Missing item title");

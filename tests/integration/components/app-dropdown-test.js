...
test('Layout using defaults', function (assert) {
  ...
  assert.ok($buttonGroup.hasClass("keep-open-no"), "Missing keep-open-no class");
...
test('Layout with options', function (assert) {
  ...
  const $buttonGroup = $dropdown.find(".btn-group");
  T.exists(assert, $buttonGroup, "Missing btn-group");
  assert.ok($buttonGroup.hasClass("btn-group-sm"), "Missing btn group size class");
  ...
  const $dropdownMenu = $dropdown.find(".dropdown-menu");
  T.exists(assert, $dropdownMenu, "Missing dropdown menu");
  T.exists(assert, $dropdownMenu.find(".dropdown-header"), "Missing dropdown header");
  assert.equal(T.text($dropdownMenu.find(".dropdown-header")), 'Select a subject', "Wrong dropdown header prompt text");
  assert.equal($dropdownMenu.find("li a.item").length, 4, "Missing anchor with item class");
  assert.equal(T.text($dropdownMenu.find("li a.item:eq(0)")), "Math", "Wrong item text");
  T.notExists(assert, $dropdownMenu.find("li.disabled"), "Disabled items should not exists");
  T.notExists(assert, $dropdownMenu.find("li.selected"), "Selected items should not exists");
...
test('Disabled/selected items by default', function (assert) {
  ...
  const items = Ember.A();
  items.addObject(DropdownItem.create({id: 1, label: "Math", disabled: true}));
  items.addObject(DropdownItem.create({id: 2, label: "Science", selected: true}));
  items.addObject(DropdownItem.create({id: 3, label: "History", disabled: true}));
  items.addObject(DropdownItem.create({id: 4, label: "Language"}));
  ...
  const $buttonGroup = $dropdown.find(".btn-group");
  T.exists(assert, $buttonGroup.find(".selected-text"), "Missing select text button");
  assert.equal(T.text($buttonGroup.find(".selected-text")), "Science", "Wrong selected text");

  //Dropdown menu
  const $dropdownMenu = $dropdown.find(".dropdown-menu");
  assert.equal($dropdownMenu.find("li a.item").length, 4, "Missing anchor with item class");
  assert.equal($dropdownMenu.find("li.disabled").length, 2, "Missing disabled items");
  assert.equal($dropdownMenu.find("li.selected").length, 1, "Missing selected items");
...
test('Single select interaction', function (assert) {
  ...
  const items = Ember.A();
  items.addObject(DropdownItem.create({id: 1, label: "Math", selected:true}));
  items.addObject(DropdownItem.create({id: 2, label: "Science", disabled:true}));
  items.addObject(DropdownItem.create({id: 3, label: "History"}));
  items.addObject(DropdownItem.create({id: 4, label: "Language"}));
  ...
  this.render(hbs`{{app-dropdown items=items onChangeAction='myChangeAction'}}`);
  ...
  const $dropdownMenu = $dropdown.find(".dropdown-menu");
  assert.equal($dropdownMenu.find("li a.item").length, 4, "Missing anchor with item class");
  assert.equal($dropdownMenu.find("li.disabled").length, 1, "Missing disabled items");
  assert.equal($dropdownMenu.find("li.selected").length, 1, "Missing selected items");
  assert.equal(T.text($dropdownMenu.find("li.selected a")), "Math", "Wrong selected item");
  ...
  assert.equal($dropdownMenu.find("li.disabled").length, 1, "Missing disabled items");
  assert.equal($dropdownMenu.find("li.selected").length, 1, "Missing selected items");
  assert.equal(T.text($dropdownMenu.find("li.selected a")), "History", "Wrong selected item");
  ...
  assert.equal($dropdownMenu.find("li.disabled").length, 1, "Missing disabled items");
  assert.equal($dropdownMenu.find("li.selected").length, 1, "Missing selected items");
  assert.equal(T.text($dropdownMenu.find("li.selected a")), "History", "Wrong selected item"); //History is still selected
...
test('Multiple select interaction', function (assert) {
  ...
  this.render(hbs`{{app-dropdown items=items onChangeAction='myChangeAction' multiple=true}}`);

  const $component = this.$(),
    $dropdown = $component.find(".app-dropdown");

  const $buttonGroup = $dropdown.find(".btn-group");
  assert.equal(T.text($buttonGroup.find(".selected-text")), "Math", "Wrong selected text");
  ...
  assert.equal($dropdownMenu.find("li a.item").length, 4, "Missing anchor with item class");
  assert.equal($dropdownMenu.find("li.disabled").length, 1, "Missing disabled items");
  assert.equal($dropdownMenu.find("li.selected").length, 2, "Missing selected items");
  assert.equal(T.text($buttonGroup.find(".selected-text")), "Math,History", "Wrong selected text");
...
test('Multiple select interaction - unselect', function (assert) {
  ...
  assert.equal($dropdownMenu.find("li a.item").length, 4, "Missing anchor with item class");
  assert.equal($dropdownMenu.find("li.disabled").length, 1, "Missing disabled items");
  assert.equal($dropdownMenu.find("li.selected").length, 0, "No selected items should be found");

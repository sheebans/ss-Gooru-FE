...
test('search-filter-default', function(assert) {
  ...
  var $component = this.$(); //component dom element
      ...
      T.exists(assert, $collectionButton.find('.icon'), "Missing icon collections button");
      assert.equal(T.text($collectionButton.find('.text')), "Collections", "Incorrect collections button text");

      const $assessmentsButton =$component.find('.assessments');
      T.exists(assert, $assessmentsButton, "Missing assessments button");
      T.exists(assert, $assessmentsButton.find('.icon'), "Missing icon assessments button");
      assert.equal(T.text($assessmentsButton.find('.text')), "Assessments", "Incorrect assessments button text");

      const $resourcesButton =$component.find('.resources');
      T.exists(assert, $resourcesButton, "Missing resources button");
      T.exists(assert, $resourcesButton.find('.icon'), "Missing icon resources button");
      assert.equal(T.text($resourcesButton.find('.text')), "Resources", "Incorrect resources button text");

      const $questionsButton =$component.find('.questions');
      T.exists(assert, $questionsButton, "Missing questions button");
      T.exists(assert, $questionsButton.find('.icon'), "Missing icon questions button");
      assert.equal(T.text($questionsButton.find('.text')), "Questions", "Incorrect questions button text");

...
moduleForComponent('gru-resource-options', 'Integration | Component | gru-resource-options', {
  integration: true,
  beforeEach: function () {
...
test('gru-question-options-default', function(assert) {
  ...
  this.render(hbs`{{search/gru-resource-options selectedOptionType=selectedOptionType}}`); //render the component
  ...
  assert.equal(T.text($videoButton.find('.text')), "Video", "Incorrect video button text");

  const $webPageButton =$menuOptions.find('.web-page');
  T.exists(assert, $webPageButton, "Missing web page button");
  T.exists(assert, $webPageButton.find('.icon'), "Missing icon  web page button");
  assert.equal(T.text($webPageButton.find('.text')), "Webpage", "Incorrect  web page button text");

  const $interactiveButton =$component.find('.interactive');
  T.exists(assert, $interactiveButton, "Missing interactive button");
  T.exists(assert, $interactiveButton.find('.icon'), "Missing icon interactive button");
  assert.equal(T.text($interactiveButton.find('.text')), "Interactive", "Incorrect interactive button text");
  ...
  assert.equal(T.text($imageButton.find('.text')), "Image", "Incorrect image button text");
  ...
  assert.equal(T.text($textButton.find('.text')), "Text", "Incorrect text button text");
  ...
  assert.equal(T.text($audioButton.find('.text')), "Audio", "Incorrect audio button text");
...
test('search-filter-onAudioClick', function(assert) {
  ...
  this.render(hbs`{{search/gru-resource-options onSelectMenuOption='selectMenuOption'}}`);
  ...
  assert.ok(!$audioOptionButton.hasClass("selected"), "Missing audio option selected");
});

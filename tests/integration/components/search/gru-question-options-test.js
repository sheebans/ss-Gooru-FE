...
test('gru-question-options-default', function(assert) {
  ...
  T.exists(assert, $freeResponseButton.find('.icon'), 'Missing icon free response button');
  assert.equal(T.text($freeResponseButton.find('.text')), 'Free Response', 'Incorrect free response button text');

...
test('it shows an error message if the selectpickers are left blank', function (assert) {
  ...
  $component.find("select.selectpicker.months").click();
  $birthDayDateField.blur();

  return wait().then(function () {
    assert.ok($birthDayDateField.find(".error-messages .error").length, 'Title error message visible');
    $birthDayDateField.find("select.selectpicker.months").val('1');
    $birthDayDateField.find("select.selectpicker.days").val('1');

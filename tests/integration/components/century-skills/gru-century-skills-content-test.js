...
moduleForComponent('century-skills/gru-century-skills-content', 'Integration | Component | century-skills/gru century skills content', {
  integration: true,
  beforeEach: function () {
...
test('Century skills content Layout - select/unselect skills', function (assert) {
  ...
  assert.equal($component.find('.column').length, 3, 'Number of skills columns');
  ...
  assert.equal($cognitiveSkillsColumn.find('li').length, 2, 'Number of skill items in the cognitive-skills columns');
  assert.equal($contentSkillsColumn.find('li').length, 1, 'Number of skill items in the content-skills columns');
  assert.equal($learningSkillsColumn.find('li').length, 2, 'Number of skill items in the learning-skills columns');
  assert.ok($component.find('.actions .cancel').length, 'Missing cancel button');
  assert.ok($component.find('.actions .select').length, 'Missing select button');
  ...
  assert.ok($skillCategories.find('.circle.hewlett').length, 'cognitive-skill hewlett category');
  assert.ok($skillCategories.find('.circle.framework').length, 'cognitive-skill framework category');

  //Skill Items selected
  assert.ok($cognitiveSkillsColumn.find('li:eq(0) .title.selected').length, 'first cognitive-skill selected');
  assert.ok($learningSkillsColumn.find('li:eq(1) .title.selected').length, 'second learning-skill selected');

  $learningSkillsColumn.find('li:eq(1) .title.selected').click();
  return wait().then(function () {
    assert.notOk($learningSkillsColumn.find('li:eq(1) .title').hasClass('selected'), 'second learning-skill should not be selected');
  });
});

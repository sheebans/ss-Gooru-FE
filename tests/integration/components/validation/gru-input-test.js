...
test('Layout and clear functionality', function(assert) {
  ...
  this.render(hbs`{{validation.gru-input model=model valuePath='term' hasClearButton=true inputId='any-id' isRequired=true}}`); // render the component
...
test('Testing events and observer', function(assert) {
  ...
  this.render(hbs`{{validation.gru-input model=model valuePath='value' hasClearButton=true}}`); // render the component
  var $component = this.$();
  var $input = $component.find("input[type=text]");
  ...
  $clear.click();
  return wait().then(function(){
    assert.equal($input.val(),"", "After a clear event the input value should be empty");

    $input.val('Hello World');
    e.which = 13; //ENTER
    $input.trigger(e);
    return wait().then(function(){
      assert.equal($input.val(),  'Hello World', 'After the Enter key has been pressed, the input value should be equal to the value property');

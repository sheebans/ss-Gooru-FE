...
moduleForComponent('content/rubric/gru-preview-url', 'Integration | Component | content/rubric/gru preview url', {
  integration: true
});
test('Preview url layout', function (assert) {
  ...
  this.render(hbs`{{content/rubric/gru-preview-url resource=resource isRubric=true}}`);

  var $component = this.$();

  assert.ok($component.find('.gru-preview-url .preview.show-legend span').length, 'Missing preview legend');

  const resource = ResourceModel.create({url:'http://www.water4all.org/us/'});
...
test('Show PDF', function (assert) {
  ...
  const resource = ResourceModel.create({url:'http://www.worldanimalfoundation.net/f/koala.pdf'});

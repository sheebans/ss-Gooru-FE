...
test('it renders correctly when there are no questions or resources', function (assert) {
  ...
  assert.equal($header.find('> .detail span').text(), this.get('i18n').t('common.add-collection-item').string, 'Resources or questions text');
  ...
  assert.ok($addDropdown.find('.dropdown-menu li:eq(0)').text(), this.get('i18n').t('common.from-my-resources').string, 'Add dropdown, first button');
  assert.ok($addDropdown.find('.dropdown-menu li:eq(1)').text(), this.get('i18n').t('common.from-my-questions').string, 'Add dropdown, second button');
  assert.ok($addDropdown.find('.dropdown-menu li:eq(2)').text(), this.get('i18n').t('common.new-resource').string, 'Add dropdown, third button');
  assert.ok($addDropdown.find('.dropdown-menu li:eq(3)').text(), this.get('i18n').t('common.new-question').string, 'Add dropdown, fourth button');

  assert.equal($header.find('.actions > button').length, 0, 'Buttons visible besides the add button');
...
test('it renders correctly when there are questions and resources', function (assert) {
  ...
  const $component = this.$('.content.collections.gru-collection-list');
  assert.ok($component.length, 'Component');
  assert.ok($component.find('> .header > .detail span').text().indexOf('1 ' + this.get('i18n').t('common.resource-pl', { count: 1}).string) > -1, 'Resource detail text');
  assert.ok($component.find('> .header > .detail span').text().indexOf('2 ' + this.get('i18n').t('common.question-pl', { count: 2}).string) > -1, 'Question detail text');

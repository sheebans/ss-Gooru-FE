...
test('HS-Image answer layout in editing mode', function(assert) {
  ...
  return wait().then(function () {
    assert.equal($component.find('.hs-container').length, 1, "Number of answers");
    ...
    assert.ok($option.find('.panel-footer .delete i.delete'), "Delete button missing");
    assert.ok($option.find('.panel-footer .check'), "Correct  button missing");
    const $check = $component.find('.panel-footer .check');
    $check.click();
    return wait().then(function () {
      assert.ok($option.find('.panel-footer .correct .check'), "The answer should be correct");
    });
  });
});
...
test('HS-Image answer view layout', function(assert) {
  ...
  this.render(hbs`{{content/questions/answers/gru-hs-image answers=answers editMode=false}}`);
  var $component = this.$(); //component dom element
  ...
  assert.ok($option.find('.panel-footer .correct .check'.length), "Correct  check icon missing");
  ...
  assert.notOk($option2.find('.panel-footer .correct .check').length, "Correct  check icon should not appear");
...
test('Correct answer', function(assert) {
  ...
  return wait().then(function () {
    ...
    assert.equal( $component.find('.check.correct').length,2, "Incorrect number of correct answer");
  });
});

...
test('Multiple answer layout', function(assert) {
  ...
  this.render(hbs`{{content/questions/answers/gru-multiple-answer answers=answers}}`);
  ...
  assert.ok($option.find('.choice-answers'), "Choices answers container missing");
  assert.equal($option.find('.choice-answers span').length, 2, "Choices answers icons missing");
});
...
test('Multiple answer layout in edit mode', function(assert) {
  ...
  this.render(hbs`{{content/questions/answers/gru-multiple-answer answers=answers editMode=true}}`);
  var $component = this.$(); //component dom element
  ...
  return wait().then(function () {
    ...
    $check.click();
    return wait().then(function () {
      assert.ok($option.find('.check.yes.correct'), "The answer should be correct");
    });
  });
});

test('Multiple answer change between yes and no in edit mode', function(assert) {
  const answers = Ember.A([Answer.create(Ember.getOwner(this).ownerInjection(), {
      'text': "Option A",
      'isCorrect': false
    })]);
  ...
  assert.notOk($yesButton.hasClass('correct'), "The yes button should not be selected");
  $yesButton.click();
  return wait().then(function () {
    assert.notOk($noButton.hasClass('correct'), "The no button should not be selected");
    assert.ok($yesButton.hasClass('correct'), "The yes button should be selected");
  });
});
...
test('Multiple answer, reach max answers', function(assert) {
  ...
  const $newAnswer = $component.find('div.add-answer a');
  assert.ok($newAnswer.length, "Add new answer choice button missing");
  assert.notOk($component.find('.max-length-error').length, "Max answers error must not exist");
  $newAnswer.click();
  return wait().then(function () {
    assert.equal($component.find('.panel').length, 10, "Number of answers");
    assert.ok($component.find('.max-length-error').length, "Max answers error must exist");

    $newAnswer.click();
    return wait().then(function () {
      assert.equal($component.find('.panel').length, 10, "Must not add new answer");
    });
  });
});

...
test('External action gets called on feedback change', function(assert) {
  ...
  var $immediate = this.$().find(`.panel-body .feedback .gru-radio input[value="${ASSESSMENT_SHOW_VALUES.IMMEDIATE}"]`);
  var $never = this.$().find(`.panel-body .feedback .gru-radio input[value="${ASSESSMENT_SHOW_VALUES.NEVER}"]`);
  assert.notOk($summary.parents('.ember-radio-button').hasClass('checked'), 'Not checked by default');
  assert.notOk($immediate.parents('.ember-radio-button').hasClass('checked'), 'Not checked by default');
  assert.ok($never.parents('.ember-radio-button').hasClass('checked'), 'Checked by default');
  ...
  assert.notOk($summary.parents('.ember-radio-button').hasClass('checked'), 'Not checked by default');
  assert.ok($immediate.parents('.ember-radio-button').hasClass('checked'), 'Checked after clicked');
  assert.notOk($never.parents('.ember-radio-button').hasClass('checked'), 'Not checked after change');
  assert.equal(this.get('model.showFeedback'), ASSESSMENT_SHOW_VALUES.IMMEDIATE, "Feedback value");
});

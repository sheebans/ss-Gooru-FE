...
moduleForComponent('content/modals/gru-delete-question', 'Integration | Component | content/modals/gru delete question', {
  integration: true,
  beforeEach: function () {
...
test('it renders', function(assert) {
  ...
  assert.equal($component.find('h4.modal-title').text(), 'Delete Question', 'Incorrect Title');
  ...
  assert.equal($component.find('.actions .delete').prop('disabled'),false, 'Delete Button Should be enable');
});
...
test('it calls a generic delete method and then redirects (if a route is provided) after clicking on the delete button', function(assert) {
  ...
  this.render(hbs`{{content/modals/gru-delete-question model=model router=router}}`);
  const $component = this.$(".gru-delete-question");
...
test('show spinner button component while the server response, after clicking on the delete button', function(assert) {
  ...
  this.render(hbs`{{content/modals/gru-delete-question model=model isLoading=isLoading}}`);
  const $component = this.$(".gru-delete-question");

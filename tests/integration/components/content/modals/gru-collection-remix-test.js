...
moduleForComponent('content/modals/collection-remix', 'Integration | Component | content/modals/gru collection remix', {
  integration: true,
...
test('it renders', function (assert) {
  ...
  assert.ok($body.find('form .gru-input.title').length, 'Collection title field');
...
test('it shows toast and transitions after copying a collection', function (assert) {
  assert.expect(6);
  ...
  this.register('service:notifications', Ember.Service.extend({
    success(message) {
      assert.notEqual(message.indexOf(
        context.get('i18n').t('common.remix-collection-success', {collectionTitle: 'Collection Name'}).string
  ...
  return wait().then(function () {
    ...
    return wait().then(function () {
      assert.equal(generatedRoute.route, 'content.collections.edit', 'Generated correct route');
      assert.equal(generatedRoute.collection, 12345, 'Correct generated route collection ID');
    });
  });
});

test('it displays a notification if the collection cannot be created', function (assert) {
  assert.expect(1);
  ...
  this.register('service:notifications', Ember.Service.extend({
    error(message) {
      assert.equal(message, context.get('i18n').t('common.errors.collection-not-copied').string, 'Notification displayed');
    }
  }));
  this.inject.service('notifications');

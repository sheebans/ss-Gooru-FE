...
moduleForComponent('content/modals/gru-rubric-new', 'Integration | Component | content/modals/gru rubric new', {
  integration: true,
  beforeEach: function () {
...
test('New Rubric Layout', function(assert) {
  ...
  assert.equal($component.find('h4.modal-title').text(), this.get('i18n').t('common.create-rubric').string, 'Incorrect Title');
  assert.ok($component.find('label span.required'), 'Missing Collection Title label');
  assert.equal($component.find('label span.required').text(), this.get('i18n').t('common.rubric-title').string, 'Incorrect Rubric Title Label');
...
test('Validate if the Rubric Title field has only whitespaces', function (assert) {
  ...
  assert.ok(!$titleField.find('.error-messages .error').length, 'Rubric Title error message not visible');

  $component.find('.actions button[type="submit"]').click();

  return wait().then(function () {

    assert.ok($titleField.find('.error-messages .error').length, 'Rubric Title error should be visible');
    $titleField.find('input').val(' ');
    $component.find('.actions button[type="submi"]').click();

    return wait().then(function () {
      assert.ok($titleField.find('.error-messages .error').length, 'Rubric Title error message should be visible');
    });
  });
});
test('Validate the character limit in the Rubric title field', function (assert) {
  ...
  const maxLenValue = this.$('.gru-rubric-new .gru-input.title input').prop('maxlength');
...
test('Create a rubric', function (assert) {
  ...
  return wait().then(function () {
    ...
    return wait().then(function () {
      assert.equal(transition.route, 'content.rubric.edit', 'Transition to correct route');
...
test('show spinner button component while the server response, after clicking on the create button', function(assert) {
  ...
  this.render(hbs`{{content/modals/gru-rubric-new router=router isLoading=isLoading }}`);

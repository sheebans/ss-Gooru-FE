...
moduleForComponent('content/modals/lesson-remix', 'Integration | Component | content/modals/gru lesson remix', {
  integration: true,
...
test('it shows an error message if the lesson title field is left blank', function (assert) {
  assert.expect(3);
...
test('it shows toast and perform event after copying a lesson', function (assert) {
  assert.expect(6);
  var context = this;
  ...
  this.register('service:notifications', Ember.Service.extend({
    success(message) {
      assert.notEqual(message.indexOf(
        context.get('i18n').t('common.remix-lesson-success', {lessonTitle: 'Lesson Name'}).string
...
test('it displays a notification if the lesson cannot be created', function (assert) {
  assert.expect(1);
  ...
  this.register('service:notifications', Ember.Service.extend({
    error(message) {
      assert.equal(message, context.get('i18n').t('common.errors.lesson-not-copied').string, 'Notification displayed');
    }
  }));
  this.inject.service('notifications');
...
test('Validate if the lesson Title field has only whitespaces', function (assert) {
  assert.expect(3);
  ...
  assert.ok(!$titleField.find(".error-messages .error").length, 'Lesson Title error message not visible');
  ...
  return wait().then(function () {
    ...
    return wait().then(function () {
      ...
      assert.ok($titleField.find(".error-messages .error").length, 'Lesson Title error should be visible');
      // Fill in the input field
      $titleField.find("input").val(' ');
      $component.find(".actions button[type='submit']").click();

      return wait().then(function () {
        assert.ok($titleField.find(".error-messages .error").length, 'Lesson Title error message should be visible');
      });
    });
  });
});

test('Validate the character limit in the lesson title field', function (assert) {
  this.set('contentModel', {
    content: LessonModel.create(Ember.getOwner(this).ownerInjection(), {
      id: 'lesson-id',
      title: 'lesson-title'
  ...
  const maxLenValue = this.$('.gru-lesson-remix .gru-input.title input').prop('maxlength');

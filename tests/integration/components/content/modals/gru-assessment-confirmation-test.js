...
moduleForComponent('content/modals/gru-add-to-lesson', 'Integration | Component | content/modals/gru assessment confirmation', {
  integration: true
});
...
test('Layout and forward only with limited attempts', function(assert) {
  ...
  assert.equal($body.find('.setting i').hasClass('trending_flat'), true, 'Wrong icon');
  assert.equal($body.find('.setting p').text().trim(), "You can navigate forward only", 'Wrong setting description');


  assert.equal($body.find('.attempts p').text().trim(), "You have 5 attempts", 'Wrong attempts description');
...
test('Navigate both ways with unlimited attempts', function(assert) {
  ...
  assert.equal($body.find('.setting i').hasClass('swap_horiz'), true, 'Wrong icon');
  assert.equal($body.find('.setting p').text().trim(), "You can navigate forward and backwards to answer questions", 'Wrong setting description');

  assert.equal($body.find('.attempts p').text().trim(), "You have unlimited attempts", 'Wrong attempts description');

...
moduleForComponent('content/modals/gru-remove-student', 'Integration | Component | content/modals/gru remove student', {
  integration: true,
  beforeEach: function () {
...
test('it renders', function(assert) {
  ...
  assert.equal($component.find('h4.modal-title').text(), this.get('i18n').t('content.modals.remove-student.title').string, 'Incorrect Title');
  assert.ok($component.find('p.legend').length, 'Missing Delete Course Legend');
  assert.ok($component.find('p.legend').text().indexOf(model.content.fullName) > -1, 'Incorrect legend');
  assert.ok($component.find('.delete-info').length, 'Missing Delete Information');
  assert.equal($component.find('.delete-info ul li:eq(0) label span').text(), this.get('i18n').t('content.modals.remove-student.data-inaccessible').string, 'Incorrect links inaccessible check');
  assert.equal($component.find('.delete-info ul li:eq(1) label span').text(), this.get('i18n').t('content.modals.remove-student.classroom-access').string, 'Incorrect student access check');
  assert.equal($component.find('.delete-info ul li:eq(2) label span').text(), this.get('i18n').t('content.modals.remove-student.data-lost').string, 'Incorrect content data deleted check');
  assert.ok($component.find('p.confirmation').length, 'Missing Delete Confirmation');
  assert.equal($component.find('p.confirmation').text(), this.get('i18n').t('content.modals.remove-student.confirmation').string, 'Incorrect Confirmation Text');
...
test('it enables the delete button under the appropriate conditions', function(assert) {
  const model = {
    content: {
      title: "Class Title"
    }

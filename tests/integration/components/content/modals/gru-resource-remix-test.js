...
moduleForComponent('content/modals/resource-remix', 'Integration | Component | content/modals/gru resource remix', {
  integration: true,
...
test('it shows toast and transitions after copying a resource', function (assert) {
  assert.expect(6);
  ...
  this.register('service:notifications', Ember.Service.extend({
    success(message) {
      assert.notEqual(message.indexOf(
        context.get('i18n').t('common.remix-resource-success', {resourceTitle: 'Resource Name'}).string
  ...
  this.render(hbs`{{content/modals/gru-resource-remix router=router model=resource}}`);
  ...
  return wait().then(function () {
    ...
    return wait().then(function () {
      assert.equal(generatedRoute.route, 'content.resources.edit', 'Generated correct route');
      assert.equal(generatedRoute.resource, 12345, 'Correct generated route resource ID');
    });
  });
});

test('it shows toast and transitions after copying a resource with parent collection', function (assert) {
  assert.expect(8);
  ...
  this.set('parentService', {
    addResource: function(parentId, resourceid) {
      assert.equal(parentId, 'collection-id', "Parent collection id when adding resource");
  ...
  this.render(hbs`{{content/modals/gru-resource-remix router=router model=resource collectionService=parentService}}`);
...
test('it displays a notification if the resource cannot be created', function (assert) {
  assert.expect(1);
  ...
  this.register('service:notifications', Ember.Service.extend({
    error(message) {
      assert.equal(message, context.get('i18n').t('common.errors.resource-not-copied').string, 'Notification displayed');
    }
  }));
  this.inject.service('notifications');
...
test('Validate the character limit in the resource title field', function (assert) {
  ...
  const maxLenValue = this.$('.gru-resource-remix .gru-input.title input').prop('maxlength');

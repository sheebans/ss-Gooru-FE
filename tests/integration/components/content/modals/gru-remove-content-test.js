...
test('it renders', function(assert) {
  ...
  assert.equal($component.find('h4.modal-title').text(), this.get('i18n').t('common.remove').string + " " + this.get('i18n').t('common.' + model.type).string, 'Incorrect Title');
  ...
  assert.ok($component.find('.remove-info').length, 'Missing Delete Information');
  assert.equal($component.find('.remove-info ul li:eq(0) label span').text(), this.get('i18n').t('common.delete-instructions.links-inaccessible').string, 'Incorrect links inaccessible check');
  assert.equal($component.find('.remove-info ul li:eq(1) label span').text(), this.get('i18n').t('common.delete-instructions.content-inaccessible').string, 'Incorrect content inaccessible check');

  assert.ok($component.find('p.confirmation').length, 'Missing Delete Confirmation');
  assert.equal($component.find('p.confirmation').text(), this.get('i18n').t('content.modals.remove-content.confirmation').string, 'Incorrect Confirmation Text');
  ...
  assert.equal($component.find('.actions .remove').prop('disabled'),true, 'Remove Button Should be disabled');
});


test('it calls a generic remove method and then a callback (if provided) after clicking on the remove button from resource', function(assert) {
  assert.expect(3);
  ...
  this.render(hbs`{{content/modals/gru-remove-content model=model validator=validator}}`);
  const $component = this.$(".gru-remove-content");

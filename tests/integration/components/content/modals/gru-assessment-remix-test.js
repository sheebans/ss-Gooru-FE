...
moduleForComponent('content/modals/assessment-remix', 'Integration | Component | content/modals/gru assessment remix', {
  integration: true,
...
test('it renders', function (assert) {
  ...
  assert.ok($body.find('form .gru-input.title').length, 'Assessment title field');
...
test('it shows toast and transitions after copying a assessment', function (assert) {
  assert.expect(6);
  ...
  this.register('service:notifications', Ember.Service.extend({
    success(message) {
      assert.notEqual(message.indexOf(
        context.get('i18n').t('common.remix-assessment-success', {assessmentTitle: 'Assessment Name'}).string
  ...
  return wait().then(function () {
    ...
    return wait().then(function () {
      assert.equal(generatedRoute.route, 'content.assessments.edit', 'Generated correct route');
      assert.equal(generatedRoute.assessment, 12345, 'Correct generated route assessment ID');
    });
  });
});

test('it displays a notification if the assessment cannot be created', function (assert) {
  assert.expect(1);
  ...
  this.register('service:notifications', Ember.Service.extend({
    error(message) {
      assert.equal(message, context.get('i18n').t('common.errors.assessment-not-copied').string, 'Notification displayed');
    }
  }));
  this.inject.service('notifications');

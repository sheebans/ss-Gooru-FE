...
test('it renders', function(assert) {
  ...
  assert.equal($component.find('h4.modal-title').text(),
    this.get('i18n').t('common.delete').string + " " + this.get('i18n').t('common.class').string, 'Incorrect Title');
  ...
  assert.equal($component.find('.delete-info ul li:eq(1) label span').text(), this.get('i18n').t('content.modals.delete-class.student-access').string, 'Incorrect student access check');
  assert.equal($component.find('.delete-info ul li:eq(2) label span').text(), this.get('i18n').t('content.modals.delete-class.student-data-deleted').string, 'Incorrect content data deleted check');
...
test('it enables the delete button under the appropriate conditions', function(assert) {
  ...
  this.render(hbs`{{content/modals/gru-delete-class model=model validator=validator}}`);
  const $component = this.$(".gru-delete-class");
...
test('it calls a generic delete method and then redirects', function(assert) {
  ...
  this.render(hbs`{{content/modals/gru-delete-class model=model validator=validator router=router}}`);
  const $component = this.$(".gru-delete-class");

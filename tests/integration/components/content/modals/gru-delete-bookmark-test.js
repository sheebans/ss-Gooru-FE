...
moduleForComponent('content/modals/gru-delete-bookmark', 'Integration | Component | content/modals/gru delete bookmark', {
  integration: true,
  beforeEach: function () {
...
test('it renders', function(assert) {
  ...
  this.render(hbs`{{content/modals/gru-delete-bookmark model=model}}`);

  const $component = this.$(".gru-delete-bookmark");
  assert.ok($component.length, 'Missing Component');
  assert.ok($component.find('h4.modal-title').length, 'Missing Title');
  assert.ok($component.find('p.confirmation').length, 'Missing Delete Course Legend');
  assert.ok($component.find('p.confirmation').text().indexOf(model.type) > -1, 'Incorrect confirmation');

...
moduleForComponent('content/modals/gru-add-to-classroom', 'Integration | Component | content/modals/gru add to classroom', {
  integration: true
});
...
test('Layout', function(assert) {
  ...
  assert.ok($container.find('.modal-header .modal-title').length, 'Missing title');
  assert.ok($container.find('.modal-body .lead').length, 'Missing lead');
  assert.equal($container.find('.list .panel').length,1, 'Missing class panel');
  assert.ok($container.find('.modal-footer .cancel').length, 'Missing cancel button');
  assert.ok($container.find('.modal-footer .add-to').length, 'Missing add to button');
});

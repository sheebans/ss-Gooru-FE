...
test('Verify the data picker options selected are in the component', function(assert) {
  ...
  T.notExists(assert, $studyTime, 'study time sub header should not be visible');
...
test('Show Report', function(assert) {
  ...
  this.render(hbs`{{class/analytics/performance/teacher/gru-metrics-sub-header dataPickerOptions=dataPickerOptions showReport=showReport}}`);
  ...
  T.exists(assert, $secondHeader.find('.report'), 'Report should appear as second Header');

...
test('it renders correctly when there are units', function(assert) {
  ...
  return wait().then(function() {
    ...
    assert.equal($items.first().find('.panel-title .title span').text().trim(), 'Unit 1: Unit 1', 'Incorrect first unit title');
    assert.equal($items.last().find('.panel-title .title span').text().trim(), 'Unit 2: Unit 2', 'Incorrect last unit title');

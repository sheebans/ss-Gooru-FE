...
test('check if selections are valid using validateSelections()', function(assert) {
  ...
  assert.ok(validator.validateSelections(answerText, 'sentence'), "Answer is valid with a sentence");

  answerText = "[The quick brown fox jumps over the lazy dog].";
  assert.notOk(validator.validateSelections(answerText, 'sentence'), "Answer is invalid due to missing point.");
});
...
test('check if answer is correct with words selection', function(assert) {
  ...
  assert.ok(validator.validate(answer.get('text'), options, answer), "Answer is correct");

  answer.set('text', 'The quick brown fox [jumps[ over the lazy dog');
  assert.equal(validator.validate(answer.get('text'), options, answer), options.wrongFormatKey, "Answer is not correct due to unbalanced brackets");

  answer.set('text', '');
  assert.equal(validator.validate(answer.get('text'), options, answer), options.answerNotSelectedKey, "Answer is not correct due to empty text");

  answer.set('text', 'The quick brown fox jumps over the lazy dog');
  assert.equal(validator.validate(answer.get('text'), options, answer), options.answerNotSelectedKey, "Answer is not correct due to not defined selections");

  answer.set('text', '[The quick brown fox jumps over the lazy dog.] The end.');
  assert.equal(validator.validate(answer.get('text'), options, answer), options.wrongFormatKey, "Answer is not correct due to sentence inside the brackets");
});
...
test('check if answer is correct with sentence selection', function(assert) {
  ...
  answer.set('text', 'The quick [brown] fox jumps over the [lazy] dog. The end.');
  assert.equal(validator.validate(answer.get('text'), options, answer), options.wrongFormatKey, "Answer is not correct due to word selections");

  answer.set('text', '[The quick brown fox jumps over the lazy dog]. The end.');
  assert.equal(validator.validate(answer.get('text'), options, answer), options.wrongFormatKey, "Answer is not correct due to missing end period in the selection");
});

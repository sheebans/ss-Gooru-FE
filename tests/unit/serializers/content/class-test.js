...
test('serializeCreateClass with custom visibility', function(assert) {
  ...
  assert.equal(response.class_sharing, 'open', 'Wrong class sharing');
  assert.equal(response.min_score, 0, 'Wrong min score');
  assert.equal(response.content_visibility, 'visible_all', 'Wrong visibility');
});
...
test('serializeUpdateClass', function(assert) {
  ...
  assert.equal(response.content_visibility, undefined, 'Wrong visibility, it should not be present');
...
test('normalizeClasses', function(assert) {
  ...
  assert.equal(normalizedClasses.get("classes")[0].get('contentVisibility'), 'visible_none', 'Wrong default visibility when receiving null');
...
test('normalizeReadClassInfo', function(assert) {
  ...
  assert.equal(normalizedClassInfo.get("contentVisibility"), 'visible_none', 'Wrong visibility when receiving null');
...
test('normalizeReadClassContentVisibility', function(assert) {
  ...
  assert.equal(classContentVisibility.get("contentVisibility"), 'visible_none', 'Wrong content visibility');

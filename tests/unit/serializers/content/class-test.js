...
test('serializeCreateClass with default visibility', function(assert) {
  ...
  assert.equal(response.content_visibility, ClassModel.VISIBLE_ALL, 'Wrong default visibility');
});
...
test('serializeCreateClass with custom visibility', function(assert) {
  ...
  assert.equal(response.class_sharing, 'open', 'Wrong class sharing');
  assert.equal(response.min_score, 0, 'Wrong min score');
  assert.equal(response.content_visibility, ClassModel.VISIBLE_ALL, 'Wrong visibility');
});
...
test('serializeUpdateClass', function(assert) {
  ...
  assert.notOk(response.content_visibility, 'Wrong visibility, it should not be present');
...
test('normalizeClasses', function(assert) {
  ...
  assert.equal(normalizedClasses.get('classes')[0].get('contentVisibility'), ClassModel.VISIBLE_NONE, 'Wrong default visibility when receiving null');
...
test('normalizeReadClassContentVisibility', function(assert) {
  ...
  const classContentVisibility = serializer.normalizeReadClassContentVisibility(payload);
  assert.equal(classContentVisibility.get('contentVisibility'), ClassModel.VISIBLE_NONE, 'Wrong content visibility');
  assert.equal(classContentVisibility.get('course'), 'fake-content', 'Wrong course');
});

...
moduleFor('serializer:analytics/current-location', 'Unit | Serializer | analytics/current-location');
...
test('normalizeCurrentLocation with collectionId, no collection type', function (assert) {
  ...
  assert.equal(location.get("collectionId"), "collection-id", 'Wrong collection id');
  assert.equal(location.get("collectionType"), "collection", 'Wrong collection type');
...
test('normalizeCurrentLocation with assessmentId, no collection type', function (assert) {
  ...
  assert.equal(location.get("lessonId"), "lesson-id", 'Wrong lesson id');
  assert.equal(location.get("collectionId"), "collection-id", 'Wrong collection id');
  assert.equal(location.get("collectionType"), "assessment", 'Wrong collection type');
  assert.equal(location.get("status"), "complete", 'Wrong class id');
});

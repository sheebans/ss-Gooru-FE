...
test('normalizeLessonInfo', function(assert) {
  ...
  assert.deepEqual(serializedData.get('children').length, 8, 'Returned data length should match');
  ...
  assert.deepEqual(serializedData.get('children')[2].title, 'A1', 'Assessment data should match');
  assert.deepEqual(serializedData.get('children')[3].title, `R1-${ASSESSMENT_SUB_TYPES.RESOURCE}`, 'Resource data should match');
  assert.deepEqual(serializedData.get('children')[4].title, 'A2', 'Assessment data should match');
  assert.deepEqual(serializedData.get('children')[5].title, `R2-${ASSESSMENT_SUB_TYPES.RESOURCE}`, 'Resource data should match');
  assert.deepEqual(serializedData.get('children')[6].title, `normalized-collection-${ASSESSMENT_SUB_TYPES.POST_TEST}`, 'Post test data should match');
  assert.deepEqual(serializedData.get('children')[7].title, `normalized-collection-${ASSESSMENT_SUB_TYPES.BENCHMARK}`, 'Benchmark data should match');
});

...
test('serializeMapContext', function (assert) {
  ...
  assert.equal(serialized.collection_id, model.get('collectionId'), 'Wrong collection id');
  assert.equal(serialized.collection_type, model.get('collectionType'), 'Wrong collection type');
  assert.equal(serialized.collection_subtype, model.get('collectionSubType'), 'Wrong collection sub type');
  assert.equal(serialized.current_item_id, model.get('itemId'), 'Wrong item id');
  assert.equal(serialized.current_item_type, model.get('itemType'), 'Wrong item type');
...
test('normalizeMapContext', function (assert) {
  ...
  assert.equal(model.get('collectionId'), payload.collection_id, 'Wrong collection id');
  assert.equal(model.get('collectionType'), payload.collection_type, 'Wrong collection type');
  assert.equal(model.get('collectionSubType'), payload.collection_subtype, 'Wrong collection sub type');
  assert.equal(model.get('itemId'), payload.current_item_id, 'Wrong item id');
  assert.equal(model.get('itemType'), payload.current_item_type, 'Wrong item type');

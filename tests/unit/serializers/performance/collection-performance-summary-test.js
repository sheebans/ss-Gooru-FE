...
moduleFor("serializer:performance/collection-performance-summary", "Unit | Serializer | performance/collection-performance-summary");
...
test("normalizeCollectionPerformanceSummary", function (assert) {
  ...
  const collectionPerformanceSummary = serializer.normalizeCollectionPerformanceSummary(data);
  assert.equal(collectionPerformanceSummary.get("id"), "123", "Wrong id");
  assert.equal(collectionPerformanceSummary.get("collectionId"), "123", "Wrong collection id");
  assert.equal(collectionPerformanceSummary.get("timeSpent"), 23, "Wrong time spent");
  assert.equal(collectionPerformanceSummary.get("score"), 10, "Wrong score");
  assert.equal(collectionPerformanceSummary.get("attempts"), 1, "Wrong attempts");
  assert.equal(collectionPerformanceSummary.get("views"), 2, "Wrong views");
  assert.equal(collectionPerformanceSummary.get("status"), "completed", "Wrong status");
});
...
test("normalizeAllCollectionPerformanceSummary", function (assert) {
  ...
  assert.equal(items[0].get("id"), "8c256e4a-4b37-423c-82b6-82fd8a128af2", "Wrong id");
});

...
test('serializeResource, for resource result', function(assert) {
  ...
  assert.equal(response.startTime, response.endTime, 'Start time and end time should be the same when end time is not provided');
...
test('serializeResource, for question result', function(assert) {
  ...
  assert.equal(response.version.logApi, '3.0', 'Wrong api log version');
  assert.deepEqual(response.metrics, {}, 'Wrong metrics');
  assert.equal(response.payLoadObject.questionType, 'MC', 'Wrong question type');
  assert.equal(response.payLoadObject.attemptStatus, 'started', 'Wrong question type');
  assert.deepEqual(response.payLoadObject.answerObject, [
    {
      "answerId": 1,
      "order": 1,
      "skip": false,
...
test('getContextValuesForResult', function(assert) {
  ...
  assert.equal(serialized.collectionType, 'collection', 'wrong collection type');
  assert.equal(serialized.resourceType, 'any-resource-type', 'wrong resource type');
  assert.equal(serialized.clientSource, 'web', 'wrong client source');
  assert.equal(serialized.reactionType, 'any-reaction-type', 'wrong reaction type');
});
...
test('serializeCollection', function(assert) {
  ...
  assert.equal(serialized.startTime, serialized.endTime, 'Start time and end time should be the same when end time is not provided');

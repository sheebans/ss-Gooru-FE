...
test('submitted', function(assert) {
  ...
  resourceResult.set("submittedAt", null);
  assert.ok(!resourceResult.get("submitted"), "It should not be submitted when null");

  //when correct is undefined
  resourceResult.set("submittedAt", undefined);
  assert.ok(!resourceResult.get("submitted"), "It should not be submitted when undefined");
});
...
test('started', function(assert) {
  ...
  assert.ok(!resourceResult.get("started"), "It should not be started when null");

  //when correct is undefined
  resourceResult.set("timeSpent", undefined);
  assert.ok(!resourceResult.get("started"), "It should not be started when undefined");
});
...
test('completed', function(assert) {
  ...
  //when correct is 0
  resourceResult.set("timeSpent", 0);
  assert.ok(!resourceResult.get("completed"), "It should not be completed when 0");

  //when correct is null
  resourceResult.set("timeSpent", null);
  assert.ok(!resourceResult.get("completed"), "It should not be completed when null");

  //when correct is undefined
  resourceResult.set("timeSpent", undefined);
  assert.ok(!resourceResult.get("completed"), "It should not be completed when undefined");
});
...
test('pending', function(assert) {
  ...
  resourceResult.set("submittedAt", null);
  assert.ok(resourceResult.get("pending"), "It should be pending when no submitted at");

  //when has not submitted at and not started at
  resourceResult.set("startedAt", null);
  assert.ok(!resourceResult.get("pending"), "It should not be pending when not started at");
});

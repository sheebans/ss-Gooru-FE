...
test('fibText having no correct answers', function (assert) {
  ...
  assert.equal(model.get("fibText"), "The ball is red and white", "Wrong FIB text");
});
...
test('isLegacyFIB', function (assert) {
  ...
  assert.ok(!model.get("isLegacyFIB"), "Should not be Legacy FIB, incorrect type");

  model.set("questionType", QUESTION_TYPES.fib);
  assert.ok(!model.get("isLegacyFIB"), "Should not be Legacy FIB, correct type but has no ______");

  model.set("text", "The car is [red]"); // new format
  assert.ok(!model.get("isLegacyFIB"), "Should not be Legacy FIB, correct type but has no ______");

  model.set("text", "The car is _______"); // legacy format
  assert.ok(model.get("isLegacyFIB"), "Should be Legacy FIB, correct type and has  ______");
...
test('updateLegacyFIBText with text following the legacy format, but no answer', function (assert) {
  ...
  assert.equal(model.get("text"), "The car is _______ and _______ and _______", "Wrong text");
});
...
test('updateLegacyFIBText with text following the legacy format and answers', function (assert) {
  ...
  assert.equal(model.get("text"), "The car is [red] and [yellow] and [white]", "Wrong text");
});

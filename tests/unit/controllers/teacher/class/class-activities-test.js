...
test('removeClassActivity', function(assert) {
  ...
  let controller = this.subject({
    ...
    classActivities: Ember.A([Ember.Object.create({
      classActivities: Ember.A([Ember.Object.create({
        id:'classActivity-id',
        collection:Ember.Object.create({
          collectionType:'collection'
  ...
  controller.set('actions.showModal', function(componentName, model) {
    assert.equal(componentName, 'content.modals.gru-remove-class-activity', 'Component name should match');
  ...
  controller.send('viewMore');
  return wait().then(function () {
    assert.equal(controller.get('classActivities').length, 2, 'Should have  2 class activity sections (today and month)');
    assert.equal(controller.get('classActivities')[0].classActivities.length, 1, 'Should have one class activity today');
    assert.equal(controller.get('classActivities')[1].classActivities.length, 1, 'Should have 1 month daily activity after view more');
    controller.send('removeClassActivity', activityToDelete);
    return wait().then(function () {
      assert.equal(controller.get('classActivities')[0].classActivities.length, 0, 'Should removed today activity');
      assert.equal(controller.get('classActivities')[1].classActivities.length, 0, 'Should removed month activity');

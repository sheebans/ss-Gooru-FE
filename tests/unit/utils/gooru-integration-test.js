...
test('routeParamsForTeacherDataPage', function (assert) {
  ...
  assert.deepEqual(params, ["class.analytics.performance.teacher.course", "any-class"], "Wrong params");
});
...
test('routeParamsForCourseMapPage with unit id', function (assert) {
  ...
  const params = integration.get("routeParamsForCourseMapPage");
  assert.deepEqual(params, ["class.overview", "any-class", { queryParams : { location : "unit1" }}], "Wrong params");
});

test('routeParamsForCourseMapPage with unit id and lesson id', function (assert) {
  const integration = GooruIntegration.create({
    params : {
      page: "course-map",
      classId: "any-class",
  ...
  const params = integration.get("routeParamsForCourseMapPage");
  assert.deepEqual(params, ["class.overview", "any-class", { queryParams : { location : "unit1-lesson1" }}], "Wrong params");
});

test('routeParamsForCourseMapPage with unit id and lesson id and collection id', function (assert) {
  const integration = GooruIntegration.create({
    params : {
      page: "course-map",
      classId: "any-class",
  ...
  const params = integration.get("routeParamsForCourseMapPage");
  assert.deepEqual(params, ["class.overview", "any-class", { queryParams : { location : "unit1-lesson1-collection1" }}], "Wrong params");
});

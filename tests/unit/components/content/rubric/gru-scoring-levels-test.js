...
test('addLevel', function(assert) {
  ...
  let level = {name:'level-test',score:9};
  component.set('scoringLevels',[level]);
  assert.equal(component.get('scoringLevels').length,1,'Scoring levels should has 1 level');
  component.send('addLevel');
  assert.equal(component.get('scoringLevels').length,2,'Scoring levels should has 2 levels');
});

...
moduleForComponent('profile/gru-edit-profile', 'Unit | Component | profile/gru edit profile', {
  integration: false
});
...
test('cancel action', function (assert) {
  ...
  let component = this.subject({
    profile: Profile.create(Ember.getOwner(this).ownerInjection(), { id: 'profile-id' }),
    router: {
      transitionTo: (transition, param) => {
        assert.equal(transition, 'profile.about', 'Transition route should match');
        assert.equal(param, 'profile-id', 'Transition profile id param should match');
        done();
      }
    }
  });
...
test('countrySelect action', function (assert) {
  ...
  component.set('tempProfile', Profile.create(Ember.getOwner(this).ownerInjection(), {
    id: 'profile-id',
    stateId: 'state-id',
    state: 'state-name',
  ...
  assert.notOk(component.get('showCountryErrorMessage'), 'Should not show country error message');
  assert.ok(component.get('showStates'), 'It should show states');
  assert.equal(component.get('districts'), 'districts', 'Districts should be mantained');
  assert.equal(component.get('countrySelected'), 'country-id', 'Country selected id should match');
  assert.equal(component.get('country'), 'country-name', 'Country name should match');
  assert.equal(component.get('stateSelected'), 'state-id', 'State selected id should match');
  assert.equal(component.get('state'), 'state-name', 'State name should match');
  assert.equal(component.get('districtSelected'), 'district-id', 'District selected id should match');
  assert.equal(component.get('otherSchoolDistrict'), 'other-district', 'Other district should match');
  ...
  assert.equal(component.get('countrySelected'), 'country-id', 'Country selected id should match');
  assert.equal(component.get('country'), 'country-name', 'Country name should match');
  assert.notOk(component.get('stateSelected'), 'State selected id should be unset');
  assert.notOk(component.get('state'), 'State name should be unset');
  assert.notOk(component.get('districtSelected'), 'District selected id should be unset');
  assert.notOk(component.get('otherSchoolDistrict'), 'Other district should be unset');
});
...
test('stateSelect action', function (assert) {
  ...
  assert.notOk(component.get('showStateErrorMessage'), 'Should not show state error message');
  assert.notOk(component.get('showDistrictErrorMessage'), 'Should not show district error message');
  assert.equal(component.get('districts'), 'districts', 'Districts should be set');
  assert.equal(component.get('stateSelected'), 'state-id', 'State selected id should match');
  assert.equal(component.get('state'), 'state-name', 'State name should match');
});
...
test('saveProfile', function(assert) {
  ...
  component.get('session.userData.username', 'username', 'Username should match');
});

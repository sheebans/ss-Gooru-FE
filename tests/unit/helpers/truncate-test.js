...
test('Truncate text passing a valid type', function(assert) {
  let result = truncate({}, { text: "Text having more than 10 characters", type: 'short'});
  assert.equal(result, "Text havin...", "Wrong textd");
});

test('Truncate text passing a valid type and max length', function(assert) {
  let result = truncate({}, { text: "Text having more than 10 characters", type: 'short', maxLength: 3 });
  assert.equal(result, "Text havin...", "Wrong text, max length should be ignored");
});

...
test('createNewPath', function (assert) {
  ...
  this.pretender.map(function () {
    this.post('/api/nucleus/v2/course-map/paths', function(request) {
      ...
      assert.equal(requestBodyJson['parent_path_id'], undefined, 'Wrong parent_path_id');
      assert.equal(requestBodyJson['target_content_type'], 'collection', 'Wrong target_content_type');
      assert.equal(requestBodyJson['target_content_subtype'], ASSESSMENT_SUB_TYPES.PRE_TEST, 'Wrong target_content_subtype');
...
test('createNewPath resource', function (assert) {
  ...
  this.pretender.map(function () {
    this.post('/api/nucleus/v2/course-map/paths', function(request) {
      ...
      assert.equal(requestBodyJson['ctx_class_id'], undefined, 'Wrong ctx_class_id');
      assert.equal(requestBodyJson['parent_path_id'], undefined, 'Wrong parent_path_id');
      assert.equal(requestBodyJson['target_content_type'], 'resource', 'Wrong target_content_type');
      assert.equal(requestBodyJson['target_content_subtype'], null, 'Wrong target_content_subtype');
      //assert.equal(requestBodyJson['target_course_id'], 'course-id', 'Wrong target_course_id');
      //assert.equal(requestBodyJson['target_unit_id'], 'unit-id', 'Wrong target_unit_id');
      //assert.equal(requestBodyJson['target_lesson_id'], 'lesson-id', 'Wrong target_lesson_id');
      assert.equal(requestBodyJson['target_collection_id'], null, 'Wrong target_collection_id');
      assert.equal(requestBodyJson['target_resource_id'], 'suggestion-id', 'Wrong target_resource_id');
      return [201, {'Content-Type': 'text/plain', 'Location': expectedResponse}, undefined];
    }, false);
  });
  let done = assert.async();

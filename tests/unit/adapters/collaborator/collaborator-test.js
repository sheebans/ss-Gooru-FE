...
test('addCollaborators', function(assert) {
  ...
  this.pretender.map(function() {
    this.put('/api/nucleus/v1/collections/10/collaborators', function(request) {
      let requestBodyJson = JSON.parse(request.requestBody);
      assert.equal(requestBodyJson.collaborators.length, 2, 'Wrong number of collaborators');
      assert.equal(requestBodyJson.collaborators[0], "1", 'Wrong collaborator at index 0');

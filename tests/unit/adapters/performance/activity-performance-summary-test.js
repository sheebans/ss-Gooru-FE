...
moduleForAdapter("adapter:performance/activity-performance-summary", "Unit | Adapter | performance/activity-performance-summary", {
  unit: true,
  beforeEach: function () {
...
test("findClassActivityPerformanceSummaryByIds", function (assert) {
  ...
  this.pretender.map(function () {
    this.post("/api/nucleus-insights/v2/class/321/activity", function (request) {
      ...
      assert.equal(requestBodyJson.collectionType, 'assessment', "Wrong collection type");
      assert.deepEqual(requestBodyJson.collectionIds, [1,2,3], "Wrong activity ids");
  ...
  adapter.findClassActivityPerformanceSummaryByIds(123, 321, [1,2,3], "assessment", new Date(2012, 0, 20), new Date(2012, 0, 30)).then(function () {

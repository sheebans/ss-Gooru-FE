...
moduleForAdapter("adapter:performance/collection-performance-summary", "Unit | Adapter | performance/collection-performance-summary", {
  unit: true,
  beforeEach: function () {
...
test("searchStudentCollectionPerformanceSummary all criteria provided", function (assert) {
  ...
  this.pretender.map(function () {
    this.get("/api/nucleus-insights/v2/study/assessment/performance", function (request) {
      assert.equal(request.queryParams.userId, "23", "Wrong user id param");
      assert.equal(request.queryParams.courseId, "course-id", "Wrong course id param");
      assert.equal(request.queryParams.unitId, "unit-id", "Wrong unit id param");
      assert.equal(request.queryParams.lessonId, "lesson-id", "Wrong lesson id param");
  ...
  adapter.searchStudentCollectionPerformanceSummary("23", {
...
test("searchStudentCollectionPerformanceSummary just required criteria provided", function (assert) {
  ...
  // Mock backend response
  this.pretender.map(function () {
    this.get("/api/nucleus-insights/v2/study/collection/performance", function (request) {
      assert.equal(request.queryParams.userId, "23", "Wrong user id param");
      assert.equal(request.queryParams.courseId, "course-id", "Wrong course id param");

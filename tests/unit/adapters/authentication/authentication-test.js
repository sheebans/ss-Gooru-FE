...
test('signOut', function(assert) {
  ...
  test('domain based redirection for  200 status', function(assert) {
    const adapter = this.subject();
    adapter.set(
      'session',
      Ember.Object.create({
    ...
    const routes = function() {
      const endpointUrl = EndPointsConfig.getEndpointSecureUrl();
      this.post(
        `${endpointUrl}/api/nucleus-auth/v2/redirect`,
        function() {
          return [
            200,
            { 'Content-Type': 'application/json' },
    ...
    adapter.domainBasedRedirection(data).then(function(response) {
      assert.deepEqual({}, response, 'Wrong response');
    });
  });

  test('domain based redirection for  303 status', function(assert) {
    const adapter = this.subject();
    adapter.set(
      'session',
      Ember.Object.create({
      ...
      this.post(
        ...
        function() {
          return [
            303,
            { 'Content-Type': 'application/json' },
            { Location: 'sliverblack.gooru.org' },
            JSON.stringify({})
          ];
        },
        false

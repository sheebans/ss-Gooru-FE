...
test('readAudiences', function(assert) {
  ...
  service.set('lookupSerializer', Ember.Object.create({
    normalizeReadAudiences: function(response) {
      assert.deepEqual(response, {}, "normalizeReadAudiences() function was called" );
      return [];
    }
  }));

  var done = assert.async();
  service.readAudiences().then(function() { done(); });
});
...
test('readLicenses', function(assert) {
  ...
  service.set('lookupSerializer', Ember.Object.create({
    normalizeReadLicenses: function(response) {
      assert.deepEqual(response, {}, "normalizeReadLicenses() function was called" );
      return [];
    }
  }));

  var done = assert.async();
  service.readLicenses().then(function() { done(); });
});
...
test('readDepthOfKnowledgeItems', function(assert) {
  ...
  service.set('lookupSerializer', Ember.Object.create({
    normalizeReadDepthOfKnowledgeItems: function(response) {
      assert.deepEqual(response, {}, "normalizeReadDepthOfKnowledgeItems() function was called" );
      return [];
    }
  }));

  var done = assert.async();
  service.readDepthOfKnowledgeItems().then(function() { done(); });
});

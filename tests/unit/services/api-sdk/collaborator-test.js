...
test('updateCollaborators', function(assert) {
  ...
  this.pretender.map(function() {
    this.put('/api/nucleus/v1/collections/10/collaborators', function() {
      return [204, {'Content-Type': 'text/plain'}, ''];
    }, false);
  });

  var done = assert.async();
  service.updateCollaborators(10, "collections", ["1"])
...
test('updateCourseCollaborators', function(assert) {
  ...
  // Pretender was included because it is needed to simulate the response Headers including the Location value
  this.pretender.map(function() {
    this.put('/api/nucleus/v1/courses/10/collaborators', function() {
      return [204, {'Content-Type': 'text/plain'}, ''];
    }, false);
  });

  var done = assert.async();
  service.updateCourseCollaborators(10, ["1"])
    .then(function(response) {
      assert.ok(response, 'Wrong response');
      done();
    });
...
test('updateCollectionCollaborators', function(assert) {
  ...
  var done = assert.async();
  service.updateCollectionCollaborators(10, ["1"])
    .then(function(response) {
      assert.ok(response, 'Wrong response');
      done();
    });
...
test('updateAssessmentCollaborators', function(assert) {
  ...
  // Pretender was included because it is needed to simulate the response Headers including the Location value
  this.pretender.map(function() {
    this.put('/api/nucleus/v1/assessments/10/collaborators', function() {
      return [204, {'Content-Type': 'text/plain'}, ''];
    }, false);
  });

  var done = assert.async();
  service.updateAssessmentCollaborators(10, ["1"])
    .then(function(response) {
      assert.ok(response, 'Wrong response');
      done();
    });

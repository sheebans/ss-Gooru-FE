...
test('findByCourseAndUnit', function (assert) {
  ...
  Ember.run(function() {
    ...
    promise.then(function(courseLocations) {
      ...
      assert.equal(locationUserLast.get('user.firstName'), 'firstname-45', 'Wrong user firstName');
      assert.equal(locationUserLast.get('user.lastName'), 'lastname-45', 'Wrong user lastName');
      assert.equal(locationUserLast.get('user.email'), 'user_45@test.com', 'Wrong user email');
      assert.equal(locationUserLast.get('user.username'), 'username-45', 'Wrong user username');
      done();
    });
  });
});

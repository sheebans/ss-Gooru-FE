...
moduleForService('service:api-sdk/suggest', 'Unit | Service | api-sdk/suggest', {
  // needs: []
});
...
test('suggestResourcesForCollectionInCourse', function(assert) {
  ...
  service.set('suggestSerializer', Ember.Object.create({
  ...
  service.suggestResourcesForCollectionInCourse('user-id', 'course-id', 'unit-id', 'lesson-id', 'collection-id', 10, 100)
...
test('suggestResourcesForCollection', function(assert) {
  ...
  var done = assert.async();
  service.suggestResourcesForCollection('user-id', 'collection-id', 10, 100)
    .then(function(response) {
      assert.equal(response, "fakeResponse", "Wrong response");
      done();
    });
...
test('suggestResourcesForResourceInCourse', function(assert) {
  ...
  var done = assert.async();
  service.suggestResourcesForResourceInCourse('user-id', 'course-id', 'unit-id', 'lesson-id', 'collection-id', 'resource-id')
    .then(function(response) {
      assert.equal(response, "fakeResponse", "Wrong response");
      done();
    });
...
test('suggestResourcesForResource', function(assert) {
  ...
  var done = assert.async();
  service.suggestResourcesForResource('user-id', 'resource-id')
    .then(function(response) {
      assert.equal(response, "fakeResponse", "Wrong response");
      done();
    });

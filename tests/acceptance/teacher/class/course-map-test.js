...
test('Layout as a teacher', function (assert) {
  ...
  andThen(function() {
    ...
    assert.equal(currentURL(), '/teacher/class/class-for-pochita-as-teacher/course-map');
...
test('Layout as a teacher with location', function (assert) {
  visit('/teacher/class/class-for-pochita-as-teacher/course-map?location=first-unit-id%2Bfirst-lesson-id');
  andThen(function() {

    assert.equal(currentURL(), '/teacher/class/class-for-pochita-as-teacher/course-map?location=first-unit-id%2Bfirst-lesson-id');
    ...
    assert.ok(!$resourceTitle.hasClass('disabled'),"Second resource should be enabled");
  });
});

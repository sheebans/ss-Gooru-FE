...
test('menu option \'activity\' is selected when navigating directly to profile.activity', function (assert) {
  visit('/profile/pochita/activity');

  andThen(function () {
    var $menu = find('.controller.profile > .navigation .profile-menu');

    assert.equal(currentURL(), '/profile/pochita/activity');
    assert.ok($menu.find('.activity').hasClass('selected'), 'Menu option \'activity\' should be selected');
  });
});
...
test('menu option \'analytics\' is selected when navigating directly to profile.analytics', function (assert) {
  visit('/profile/pochita/analytics');

  andThen(function () {
    var $menu = find('.controller.profile > .navigation .profile-menu');

    assert.equal(currentURL(), '/profile/pochita/analytics');
    assert.ok($menu.find('.analytics').hasClass('selected'), 'Menu option \'analytics\' should be selected');
  });
});
  ...
  andThen(function () {
    ...
    andThen(function () {
      ...
      andThen(function () {
        ...
        assert.ok($menu.find('.analytics').hasClass('selected'), 'Menu option \'analytics\' should now be selected');
      });
    });
  });
});

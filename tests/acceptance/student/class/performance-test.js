...
test('My report', function (assert) {
  ...
  andThen(function() {
    ...
    andThen(function() {
      let $assessmentColumn = find('.gru-performance-table thead tr th.assessment');
      assert.ok($assessmentColumn.text().includes('Assessment'),'Incorrect column name, should say assessment');
    });
  });
});

...
test('It shows an error message if the End Date field is left blank', function(assert) {
...
test('It shows an error message if the Start Date is greater than End Date', function(assert) {
...
test('It shows an error message if the Status field is not selected', function(assert) {
  ...
  andThen(function() {
    ...
    andThen(function() {
      ...
      click($form.find('.create-goal'));
      andThen(function () {
        assert.ok($statusField.find(".error-messages .error").length, 'Status error message is visible');
        assert.equal(T.text($statusField.find(".error-messages .error")), 'Please select the Goal Status', 'Status error message is correct');

        //valid
        $statusField.find(".selectpicker").val('completed').change();
        click($form.find('.create-goal'));
        andThen(function () {
          T.notExists(assert, $statusField.find(".error-messages span.error"), 'Error message for status should not be visible');
        });
      });
    });
  });

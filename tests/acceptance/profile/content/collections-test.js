...
test('Add to daily class activities', function(assert) {
  ...
  andThen(function() {
    ...
    const $contentCollectionsContainer = find('.controller.content-collections');
    T.exists(assert, $contentCollectionsContainer, 'Missing content collections container');

    const $addTo = $contentCollectionsContainer.find('.collections .card:nth-child(1) .add-btn');
    click($addTo);
    andThen(function() {

...
test('Legacy uri collection-play with no content id', function(assert) {
  ...
  andThen(function() {
    assert.equal(
      currentURL(),
      '/player/all-resource-types-collection-id?resourceId=image-resource-id&role=undefined&type=collection'
    );
  });
});
...
test('Legacy uri collection-play with content id', function(assert) {
  ...
  andThen(function() {
    assert.equal(
      currentURL(),
      '/player/all-resource-types-collection-id?resourceId=f86f874c-efc9-4100-9cf7-55eb86ec95ae&role=undefined&type=collection'
    );
  });
});
...
test('Legacy uri assessment-play with no content id', function(assert) {
  ...
  andThen(function() {
    assert.equal(
      currentURL(),
      '/player/all-question-types-assessment-id?resourceId=image-resource-id&role=undefined&type=assessment'
    );
  });
});
...
test('Legacy uri assessment-play with content id', function(assert) {
  ...
  andThen(function() {
    assert.equal(
      currentURL(),
      '/player/all-question-types-assessment-id?resourceId=f86f874c-efc9-4100-9cf7-55eb86ec95ae&role=undefined&type=assessment'
    );
  });
});

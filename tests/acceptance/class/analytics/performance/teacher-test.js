...
test('Layout', function(assert) {
  ...
  andThen(function() {
    ...
    T.exists(assert, $performanceContainer.find('.gru-filters .performance-scale .grading-scale-legend'), 'Missing grading-scale-legend helper');
...
test('When view by assessment and collection option is selected for course level', function(assert) {
   ...
   andThen(function() {
    ...
    andThen(function() {
      ...
      assert.equal($metricsTable.find('.sub-header.hidden-xs .gru-metrics-sub-header:eq(2) .metrics-sub-header .time-spent').length,1, 'The table should filter by study time');
    });
   });
});
...
test('When view by assessment and collection option is selected for lesson level', function(assert) {
  ...
  andThen(function() {
    ...
    andThen(function() {
      ...
      assert.equal($metricsTable.find('.sub-header.hidden-xs .gru-metrics-sub-header:eq(1) .metrics-sub-header .time-spent').length,1, 'The table should filter by study time');
